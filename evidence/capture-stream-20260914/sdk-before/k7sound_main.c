...
{
  return sound_run(argc, argv, NULL, NULL, 0, 0);
}
                      ...
                      unsigned capture_frames)
{
  ...
  return sound_run(2, argv, pcm, NULL, frames, capture_frames) ? -EIO : 0;
}
                         ...
                         unsigned capture_frames)
{
  if (!pcm || !frames || frames > PIO_PLAYBACK_FRAMES ||
      capture_frames > PIO_FRAMES) return -EINVAL;
  char *argv[] = {(char *)"k7sound", (char *)"speech-internal"};
  return sound_run(2, argv, NULL, pcm, frames, capture_frames) ? -EIO : 0;
}
