 ...
 if(!fault)
  {
   ...
   unsigned int lifetime = video_mode ? 3600 : 600;
   printf("LINK running %us mode=%s, private VID/PID 1209:0001\n",
          lifetime, video_mode?"JPEG video":"Bulk test");
   uint64_t start=now(NULL);
   while(!fault && now(NULL)-start<(uint64_t)lifetime*1000000 && (video_mode || bulk_count<1024))
    {
