...
void k7_pipeline_halt(void) { atomic_store(&g_track_halt, true); }
#endif
      ...
      if (p->tracking)
        {
          ...
          if(p->photo)k7_photo_process(p->photo,slot->data,slot->bytes,slot->sequence,
  ...
  if (track)
    {
      int x=0, y=0;
      p->tracking=true;
      if(track->photo) {p->photo=k7_photo_create();if(!p->photo){ret=ENOMEM;goto free_storage;}}
      atomic_store(&g_track_halt, false);
      if (track->transmit)
        {
...
#ifdef __NuttX__
  /* Capture application priority is 100, USB HPWORK priority is 192.
   * Lower decoder priority keeps bank refill ahead of image processing.
   */
  scheduling.sched_priority = 90;
