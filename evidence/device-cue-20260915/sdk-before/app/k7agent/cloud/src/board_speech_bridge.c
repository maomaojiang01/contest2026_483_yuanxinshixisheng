...
{
  struct live *p=arg;
  p->capture_rc=p->chat?k7sound_capture_stream_cued(48000,k7_stream_ring_sample,&p->ring):
    k7sound_capture_stream(48000,k7_stream_ring_sample,&p->ring);
...
static int photo_upload(const char *ip,unsigned command_epoch,uint32_t photo_epoch)
{
  ...
  if(!rc){fd=connect_server(ip);if(fd<0)rc=fd;}
  if(!rc)
    {
      pthread_mutex_lock(&tts_lock);
  ...
  if(!rc)rc=transfer(fd,(void *)"K7S1U",5,1);
  char capture_id[80];
  snprintf(capture_id,sizeof(capture_id),"k7-%016" PRIx64 "-%" PRIu32,boot_id,photo_epoch);
  if(!rc)rc=send_packet(fd,'M',capture_id,strlen(capture_id));
  ...
  for(unsigned i=0;!rc && i<3;i++)
    {
      ...
      if(!rc)rc=transfer(fd,images+i*cap,n,1);
    }
  if(!rc)rc=k7_photo_native_status(&current,&mask);
  if(!rc && (current!=photo_epoch || mask!=7 || command_epoch!=atomic_load(&device_epoch)))rc=-ECANCELED;
  if(!rc)rc=k7_pipeline_voice_mode_status(0);
...
int k7cloud_device_loop(const char *ip,unsigned rounds)
{
  ...
  struct k7_photo_prompt_state photos={0};
  for(unsigned round=0;round<rounds;round++)
    {
      ...
      if(rc)break;
      rc=photo_prompts(ip,epoch,&photos);
      if(rc)break;
      usleep(250000);
    }
  k7cloud_device_stop();
