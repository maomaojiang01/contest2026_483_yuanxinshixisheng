  ...
  for(size_t i=0;text[i];)
    {
      ...
      normalized[used++]=text[i++];
    }
  normalized[used]=0;
  if(!strcmp(normalized,"启动云台") || !strcmp(normalized,"开启云台"))return K7_VOICE_TRACK;
  /* Exact observed ASR homophone only; never substring/fuzzy matching. */
  if(!strcmp(normalized,"测肤") || !strcmp(normalized,"开始测肤") ||
     !strcmp(normalized,"开始侧敷"))return K7_VOICE_PHOTO;
