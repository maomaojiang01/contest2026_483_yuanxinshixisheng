...
#define GGML_LOCAL_UNUSED GGML_UNUSED
...
#if defined(_WIN32) && !defined(GGML_POOL_POSIX)
...
#define ggml_lock_init(x)    GGML_LOCAL_UNUSED(x)
#define ggml_lock_destroy(x) GGML_LOCAL_UNUSED(x)
#if defined(__x86_64__) || (defined(_MSC_VER) && defined(_M_AMD64))
#define ggml_lock_lock(x)    _mm_pause()
#else
#define ggml_lock_lock(x)    GGML_LOCAL_UNUSED(x)
#endif
#define ggml_lock_unlock(x)  GGML_LOCAL_UNUSED(x)
...
static void ggml_vec_dot_f32(int n, float * GGML_RESTRICT s, size_t bs, const float * GGML_RESTRICT x, size_t bx, const float * GGML_RESTRICT y, size_t by, int nrc) {
   assert(nrc == 1);
   GGML_LOCAL_UNUSED(nrc);
   GGML_LOCAL_UNUSED(bx);
   GGML_LOCAL_UNUSED(by);
   GGML_LOCAL_UNUSED(bs);
...
static void ggml_compute_forward_add1_f32(
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        GGML_LOCAL_UNUSED(ggml_vec_add1_f32);
...
static void ggml_compute_forward_mean_f32(
    ...
    GGML_LOCAL_UNUSED(ne0);
    GGML_LOCAL_UNUSED(ne1);
    GGML_LOCAL_UNUSED(ne2);
    GGML_LOCAL_UNUSED(ne3);
...
static void ggml_compute_forward_reshape(
        ...
        struct ggml_tensor * dst) {
    // NOP
    GGML_LOCAL_UNUSED(params);
    GGML_LOCAL_UNUSED(dst);
}
...
static void set_numa_thread_affinity(int thread_n) { GGML_LOCAL_UNUSED(thread_n);  }
static void clear_numa_thread_affinity(void) {}
#endif
...
static int pool_fault(const struct ggml_pool_hooks *,enum ggml_pool_site,unsigned);
...
static thread_ret_t ggml_graph_compute_secondary_thread(void* data) {
    ...
    pool_note(&threadpool->safe_hooks,POOL_RUNNING,1,(size_t)state->ith);
    ...
    while (true) {
        ...
        if (state->pending) {
            ...
            pool_note(&threadpool->safe_hooks,POOL_COMPUTE,1,(size_t)state->ith);
...
void ggml_cpu_init(void) {
    ...
    if (is_first_call) {
        ...
        {
            const uint64_t t_start = ggml_time_us(); GGML_LOCAL_UNUSED(t_start);
            ...
            const uint64_t t_end = ggml_time_us(); GGML_LOCAL_UNUSED(t_end);
