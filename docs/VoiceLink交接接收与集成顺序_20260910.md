...
用户已提供接线照片并报告官方喇叭/麦克风插入。照片线束交叠，麦克风是否插MIC而非FAN待确认；未使能codec、未录放音。parallel-neon-probe-medium/audio-bringup-gap-v1继续冻结资料核对与纯PCM测试组件；保持已有codec NOT_READY门禁。主会话继续USB存储，不并行操作音频硬件。

用户随后明确确认“MIC接了麦克风”，接线归属按用户确认更新；照片遮挡不再作为纯代码开发阻塞。实际录音仍待驱动与真机验证。三个隔离代理分别推进I2C3控制、codec有据配置、SAI1路径，主会话独占板子，按顺序接入。

最新接线近照可追踪喇叭线至SPK接口，用户已明确MIC接麦克风，保持接线。audio-codec-datasheet-v1取得ES8388原厂用户指南的Radxa镜像；audio-i2c3-adapter-v1、audio-sai1-adapter-v1与PCM观察组件均为主机候选，未完成目标驱动或录放音验收。硬件未就绪状态未绕过。
