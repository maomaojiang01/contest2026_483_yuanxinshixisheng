...
async def bounded(awaitable, seconds=10):
...
async def relay(client, upstream, task_id, model):
    ...
    async def send_audio():
        seq = 0
        while True:
            message = await bounded(client.receive(), 10)
            ...
            if raw is not None:
                ...
                await bounded(upstream.send(raw[4:]), 5)
                seq += 1
    ...
    async def receive_results():
        revision = 0
        while True:
            kind, payload = parse_event(await bounded(upstream.recv(), 15), task_id)
