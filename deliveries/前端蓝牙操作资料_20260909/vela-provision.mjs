...
export class VelaProvision {
  constructor(uniApi, { onConnection = () => {}, onAccessPoint = () => {} } = {}) {
  ...
  _message(message) {
    ...
    if (message.event === 'error') { this._reject(Object.assign(new Error(message.code), { code: message.code })); return; }
  ...
  scan() { return this._request('X', { id: null, scan: true }); }
  status() {
    const id = this.nextId = this.nextId % 65535 + 1;
    return this._request(JSON.stringify({ v: 1, id, cmd: 'status' }) + '\n', { id, timeout: 15000 });
  }
  // This native version does not implement WPA/IP. Do not collect/send a real
  // password until the later negotiated capability explicitly permits it.
  connectWifi() { return Promise.reject(Object.assign(new Error('not_ready'), { code: 'not_ready' })); }
  submitCredentials({ ssid_b64, password } = {}) {
    if (!this.connected || this.capabilities?.receive_credentials !== true || !Number.isInteger(this.capabilities.session))
    ...
    const id = this.nextId = this.nextId % 65535 + 1;
    return this._request(JSON.stringify({ v: 1, id, cmd: 'submit_credentials', ssid_b64, password }) + '\n',
      { id, expected: 'credentials_received', session: this.capabilities.session, timeout: 15000 });
  }
  async disconnect() {
