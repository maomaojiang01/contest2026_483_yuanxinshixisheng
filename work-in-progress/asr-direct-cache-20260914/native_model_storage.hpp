...
class ModelStorage final {
 ...
 public:
  ...
  ModelStorage() : runtime(sr_shared::shared_model_runtime()) {
  ...
  void initialize(OrtEnv *environment, OrtSessionOptions *options) {
    ...
    // The transfer protocol already places immutable, hash-checked model bytes
    // outside the allocator window. ORT is configured to retain those bytes, so
    // copying another 238 MiB into the 512 MiB arena only starves stream state.
    encoder_bytes = const_cast<void *>(source_encoder);
    decoder_bytes = const_cast<void *>(source_decoder);
    std::printf("ASR_STORAGE direct encoder=%p decoder=%p available=%zu\n",
                encoder_bytes, decoder_bytes, k7_model_available());
    std::fflush(stdout);
