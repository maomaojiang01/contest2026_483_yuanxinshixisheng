...
extern "C" void k7_asr_runtime_reset(void) {
  if (!g_recognizer) return;
  SherpaOnnxDestroyOnlineRecognizer(g_recognizer);
  g_recognizer = nullptr;
  std::puts("ASR_CACHE reset=1");
  std::fflush(stdout);
}
...
extern "C" int k7_asr_model_session_probe(void) {
  ...
  const k7_asr_audio_request request{samples.get(), test_frames, kSampleRate,
                                     0, nullptr, nullptr};
  int result = recognize_audio(request, text, sizeof(text));
  std::printf("ASR_MODEL iteration=1 result=%d frames=%u bytes=%u\n", result,
              test_frames, result ? 0u : unsigned(std::strlen(text)));
  if (result) return result;
  text[0] = '\0';
  result = recognize_audio(request, text, sizeof(text));
  std::printf("ASR_MODEL iteration=2 result=%d frames=%u bytes=%u\n", result,
              test_frames, result ? 0u : unsigned(std::strlen(text)));
  return result;
}
