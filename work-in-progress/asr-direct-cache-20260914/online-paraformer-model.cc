...
#ifdef K7_ASR_NATIVE_STORAGE
#include "native_model_storage.hpp"
#endif
...
class OnlineParaformerModel::Impl {
 public:
  explicit Impl(const OnlineModelConfig &config)
      : config_(config),
        ...
        allocator_{} {
#ifdef K7_ASR_NATIVE_STORAGE
    native_storage_.initialize(env_, sess_opts_);
    InitEncoder(native_storage_.encoder_bytes, native_storage_.encoder_size);
    InitDecoder(native_storage_.decoder_bytes, native_storage_.decoder_size);
#else
    {
...
#ifdef K7_ASR_NATIVE_STORAGE
    return native_storage_.runtime.allocator();
#else
    return allocator_;
#endif
  }
 ...
 private:
  ...
  // Must outlive Env and Sessions: declaration order is intentional.
#ifdef K7_ASR_NATIVE_STORAGE
  k7_asr::ModelStorage native_storage_;
#endif
  Ort::Env env_;
