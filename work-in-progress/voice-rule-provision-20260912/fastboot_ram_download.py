...
def claim_fastboot_interface(device, interface_number, timeout_seconds=5.0):
    """Claim a newly redirected interface after transient guest-side probing."""
    import usb.core
    import usb.util
    ...
    while True:
        ...
        try:
            ...
            if error.errno != 16 or time.monotonic() >= deadline:
                raise
            time.sleep(0.1)
...
def main():
    ...
    try:
        record['claim_attempts'] = claim_fastboot_interface(
            device, interface.bInterfaceNumber)
