...
HERE = Path(os.environ.get("K7VOICE_PACKAGE_DIR", SCRIPT_DIR)).resolve()
MANIFEST = Path(os.environ.get(
    "K7VOICE_MANIFEST", HERE / "prefix-package.json")).resolve()
PAYLOAD = Path(os.environ.get(
    "K7VOICE_PAYLOAD", HERE / "spoken-tts-prefix.bin")).resolve()
MODE = sys.argv[1]
...
try:
    if MODE in ("stage-rom", "stage-rom-noreset"):
        ...
        with open_port() as port:
            ...
            if b"nsh>" not in read(port, 30):
                # SMP startup messages can obscure the first console prompt.
                # A blank line resynchronizes NSH without rebooting or retrying
                # any hardware command. Preserve the first raw output.
                port.write(b"\r")
                port.flush()
                if b"nsh>" not in read(port, 5):
