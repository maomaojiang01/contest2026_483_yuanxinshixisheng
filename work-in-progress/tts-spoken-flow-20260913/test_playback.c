...
static int on(void*x){struct sim*s=x;assert(s->reg[4]&12);if(s->reg[4]&4)assert((s->reg[0] & (3u<<20))==0);s->on++;return s->mode==2?-1:0;}
...
int main(void){struct sim s={0};struct pio_result r;static uint32_t data[2*PIO_PLAYBACK_FRAMES];
 ...
 memset(&s,0,sizeof s);s.source=data;s.limit=2*PIO_PLAYBACK_FRAMES;
 assert(!pio_play_buffer(&p,1,4096000,data,s.limit,PIO_PLAYBACK_FRAMES,&r));
 assert(r.frames==PIO_PLAYBACK_FRAMES && s.words==s.limit && !r.held);
 memset(&s,0,sizeof s);
 assert(pio_play_buffer(&p,1,4096000,data,~0u,PIO_PLAYBACK_FRAMES+1,&r)==-22&&!s.writes);
 puts("PASS 30-second speech playback, exact words and cleanup; original capture bounds preserved");return 0;}
