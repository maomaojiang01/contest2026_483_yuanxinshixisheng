...
static int sound_run(int argc, char **argv, const uint32_t *speech,
                     unsigned speech_frames, unsigned listen_frames)
{
  ...
  bool maximum = speech || cued || ma4 || hp80 || !strcmp(argv[1], "replay-max") || !strcmp(argv[1], "tone-max");
  ...
  if (!rc && loopback)
  ...
  else if (!rc && !probe)
    {
      ...
      if (cued)
        {
          playing = true;
          rc = speech ? pio_play_buffer(&pio, 1, 4096000, speech,
      ...
      if (!rc) rc = speech && !cued ? pio_play_buffer(&pio, 1, 4096000, speech, 2 * speech_frames, frames, &result) :
...
{
  return sound_run(argc, argv, NULL, 0, 0);
}
                      ...
                      unsigned capture_frames)
{
  ...
  return sound_run(2, argv, pcm, frames, capture_frames) ? -EIO : 0;
}
