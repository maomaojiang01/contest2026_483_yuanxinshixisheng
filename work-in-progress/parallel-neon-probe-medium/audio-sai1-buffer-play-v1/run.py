...
V=R.parent/'audio-sai1-pio-v3'
manifest=[]
for i,n in enumerate(['pio.c','pio.h','input/rockchip_sai.h','inputs.json','HANDOFF.md']):
 p=V/n;data=p.read_bytes();dest=R/'input'/('%02d_'%i+p.name)
 if dest.exists() and dest.read_bytes()!=data:raise SystemExit('Frozen input changed')
 dest.write_bytes(data);manifest.append({'source':str(p),'snapshot':dest.name,'sha256':hashlib.sha256(data).hexdigest()})
(R/'inputs.json').write_text(json.dumps(manifest,indent=2))
patch=''
for i,n in enumerate(['pio.c','pio.h']):
 old=(R/'input'/('%02d_'%i+n)).read_text().splitlines(True)
 patch+=''.join(difflib.unified_diff(old,(R/n).read_text().splitlines(True),fromfile='a/app/k7sound/'+n,tofile='b/app/k7sound/'+n))
(R/'buffer-play.patch').write_text(patch)
