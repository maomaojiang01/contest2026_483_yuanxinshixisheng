...
#include "usbhost_xhci_rk3576.h"
#include "usbhost_xhci_rk3576_trace.h"
...
struct k7_flow_bank_s
{
  uint8_t *buffer;
  int32_t *results;
  unsigned int done;
  unsigned int state; /* 1 DMA, 2 ready, 3 CPU */
  bool gap;
};
...
struct xhci_epinfo_s
{
  ...
  size_t             batch_packet;
  bool               batch_active;
  bool               batch_pending_wait;
  bool               batch_failed; /* Timeout requires endpoint reset */
  bool               flow_mode;
  bool               flow_owned;
  bool               flow_started;
  unsigned int       flow_inflight;
  unsigned int       flow_refill_gaps;
  unsigned int       flow_empty_events;
  size_t             flow_packet;
  struct k7_flow_bank_s flow_bank[K7_FLOW_BANKS];
  struct xhci_trb_s  *flow_trbs;
};
                                ...
                                uint64_t value)
{
  ...
  __asm__ __volatile__("" : "+r"(value));
  /* Match xHCI's low-dword then high-dword pointer register access.
...
static int xhci_ring_init(FAR struct xhci_ring_s *ring, size_t len)
{
  ...
  trb->d0 = htole64(k7_xhci_va_to_pa(&ring->ring[0]));
  trb->d1 = 0;
  trb->d2 = 0;
...
static int xhci_ctrl_start(FAR struct usbhost_xhci_s *priv)
{
  ...
  priv->pg_ctx[0] = priv->pg_sb ?
                    htole64(k7_xhci_va_to_pa(priv->pg_sb)) : 0;

  /* Device Context Base Address Array Pointer */

  xhci_oper_putreg_8b(priv, XHCI_DCBAAP, k7_xhci_va_to_pa(priv->pg_ctx));
  ...
  evnt->base = htole64(k7_xhci_va_to_pa(priv->evnt.ring));
  ...
  xhci_runt_putreg_8b(priv, XHCI_ERDP(0),
                      k7_xhci_va_to_pa(priv->evnt.ring));
...
static int xhci_address_set(FAR struct usbhost_xhci_s *priv,
                            FAR struct xhci_dev_s *dev, bool setaddr)
{
  uint64_t ctx = k7_xhci_va_to_pa(dev->input);
                          ...
                          FAR struct xhci_dev_s *dev)
{
  ...
  drdp = k7_xhci_va_to_pa(dev->epinfo[0]->td.ring);
  ...
  xhci_dcbaa_set(priv, dev->slot, k7_xhci_va_to_pa(dev->ctx));
                        ...
                        FAR struct xhci_trb_s *trb, uint16_t timeout_ms)
{
  ...
  if (ret < 0)
    {
      uerr("K7 BUS: CFG0=%08" PRIx32 " CFG1=%08" PRIx32 "\n",
      ...
      xhci_events_poll(priv);
      uerr("K7 CMD timeout: ret=%d irq=%" PRIu32 " op=%" PRIxPTR
             " rt=%" PRIxPTR " db=%" PRIxPTR "\n", ret, g_k7_irq_count,
             priv->oper_base, priv->runt_base, priv->door_base);
      uerr("K7 REGS: STS=%08" PRIx32 " CRCR=%08" PRIx32
      ...
      uerr("K7 EVENT: IMAN=%08" PRIx32 " SIZE=%08" PRIx32
      ...
      uerr("K7 DMA: cmd=%p erst=%p event=%p i=%zu c=%u\n",
             priv->cmd.ring, priv->pg_erst, priv->evnt.ring,
             priv->evnt.i, priv->evnt.ccs);
      for (int k = 0; k < 4; k++)
        uerr("K7 EVT%d %016" PRIx64 " %08" PRIx32 " %08" PRIx32 "\n",
               k, priv->evnt.ring[k].d0, priv->evnt.ring[k].d1,
               priv->evnt.ring[k].d2);
      uerr("K7 CMD0 %016" PRIx64 " %08" PRIx32 " %08" PRIx32 "\n",
                              ...
                              FAR uint8_t *buffer, size_t buflen)
{
  ...
  /* OUT direction is encoded as zero, so it cannot be tested with &.
  ...
  if (buffer && buflen)
    {
      ...
      trb[i].d0 = k7_xhci_va_to_pa(buffer);
                           ...
                           FAR uint8_t *buffer, size_t buflen)
{
  ...
  trb.d0 = k7_xhci_va_to_pa(buffer);
                                   ...
                                   FAR struct xhci_trb_s *evt)
{
  ...
  /* Ring-empty events never complete a TD. Count them for the queued
   * diagnostic without consuming any semaphore or result entry. */
  if (ret == XHCI_TRB_CC_RUR || ret == XHCI_TRB_CC_ROV)
    {
      if (epinfo->flow_mode) epinfo->flow_empty_events++;
      return;
    }
  ...
  if (epinfo->flow_mode)
    {
      ...
      unsigned int index, bank;
      struct k7_flow_bank_s *b;
      if (address < base || (address - base) % sizeof(struct xhci_trb_s)) return;
      index = (address - base) / sizeof(struct xhci_trb_s);
      if (index >= K7_FLOW_TOTAL) return;
      bank = index / K7_FLOW_SLOTS;
      index %= K7_FLOW_SLOTS;
      b = &epinfo->flow_bank[bank];
      if (b->state != 1 || b->results[index] != -EINPROGRESS) return;
      b->results[index] =
        (ret == XHCI_TRB_CC_SUCCESS || ret == XHCI_TRB_CC_SHORT_PKT) &&
        tl <= epinfo->flow_packet ? epinfo->flow_packet - tl : -EIO;
      if (++b->done == K7_FLOW_SLOTS)
        {
          b->state = 2;
          epinfo->flow_inflight--;
...
/* Bounded diagnostic queue: eight 256-TRB banks fill the existing ring.
 * One task owns exclsem from start through end. A completed bank is read
 * by the CPU, then refilled while the other bank remains in flight.
 * No endpoint or DMA memory is freed until board reset.
 */
static void k7_flow_publish(struct usbhost_xhci_s *priv,
                           struct xhci_epinfo_s *info, unsigned int bank)
{
  struct k7_flow_bank_s *b = &info->flow_bank[bank];
  irqstate_t flags;
  up_invalidate_dcache((uintptr_t)b->buffer,
                       (uintptr_t)b->buffer + K7_FLOW_SLOTS * 4096);
  for (unsigned int i = 0; i < K7_FLOW_SLOTS; i++)
    {
      b->results[i] = -EINPROGRESS;
      info->flow_trbs[i].d0 = k7_xhci_va_to_pa(b->buffer + i * 4096);
      info->flow_trbs[i].d1 = XHCI_TRB_D1_TXLEN_SET(info->flow_packet);
      info->flow_trbs[i].d2 = XHCI_TRB_D2_IOC | XHCI_TRB_D2_ISP |
        XHCI_TRB_D2_SIA | XHCI_TRB_D2_TYPE_SET(XHCI_TRB_TYPE_ISOCH);
    }

  flags = enter_critical_section();
  b->gap = info->flow_inflight == 0;
  if (b->gap && info->flow_started) info->flow_refill_gaps++;
  info->flow_started = true;
  b->done = 0;
  b->state = 1; /* DMA owns this bank. */
  info->flow_inflight++;
  leave_critical_section(flags);
  xhci_add_trb(priv, &info->td, info->flow_trbs, K7_FLOW_SLOTS);
}

int k7_xhci_flow_start(struct usbhost_driver_s *drvr, usbhost_ep_t ep,
                       uint8_t **buffers, size_t packet, int32_t **results)
{
  struct usbhost_xhci_s *priv = XHCI_PRIV_FROM_DRVR(drvr);
  struct xhci_epinfo_s *info = (struct xhci_epinfo_s *)ep;
  int ret;

  if (!info || !buffers || !results || packet == 0 || packet > 3072 ||
      info->xfrtype != USB_EP_ATTR_XFER_ISOC || !info->dirin ||
      info->td.i != 0 || info->td.len != 2049) return -EINVAL;
  for (unsigned int i = 0; i < K7_FLOW_BANKS; i++)
    {
      if (!buffers[i] || !results[i] || (uintptr_t)buffers[i] % 4096 ||
          !xhci_dmacapable(priv, buffers[i], K7_FLOW_SLOTS * 4096)) return -EINVAL;
    }
  for (unsigned int i=0;i<K7_FLOW_BANKS;i++)
    for (unsigned int j=0;j<i;j++)
      if (buffers[i]==buffers[j] || results[i]==results[j]) return -EINVAL;
  ret = nxmutex_lock(&info->exclsem);
  if (ret < 0) return ret;
  ret = nxmutex_lock(&priv->lock);
  if (ret < 0) { nxmutex_unlock(&info->exclsem); return ret; }
  if (info->iocwait || info->batch_active || info->batch_failed ||
      info->flow_mode)
    { ret = -EBUSY; goto fail; }
  info->flow_trbs = kmm_zalloc(K7_FLOW_SLOTS * sizeof(struct xhci_trb_s));
  if (!info->flow_trbs) { ret = -ENOMEM; goto fail; }
  for (unsigned int i = 0; i < K7_FLOW_BANKS; i++)
    {
      info->flow_bank[i].buffer = buffers[i];
      info->flow_bank[i].results = results[i];
    }
  info->flow_mode = true;
  info->flow_owned = true;
  info->flow_packet = packet;
  for (unsigned int i=0;i<K7_FLOW_BANKS;i++)
    k7_flow_publish(priv, info, i);
  xhci_ep_doorbell(priv, info);
  nxmutex_unlock(&priv->lock);
  return OK; /* Paired end releases exclsem. */
fail:
  nxmutex_unlock(&priv->lock);
...
int k7_xhci_flow_wait(usbhost_ep_t ep, unsigned int bank, bool *gap)
{
  struct xhci_epinfo_s *info = (struct xhci_epinfo_s *)ep;
  irqstate_t flags;
  int ret;
  if (!info || bank >= K7_FLOW_BANKS || !gap || !info->flow_owned) return -EINVAL;
  ret = nxsem_tickwait_uninterruptible(&info->iocsem, SEC2TICK(2));
  if (ret < 0) { info->batch_failed = true; return ret; }
  flags = enter_critical_section();
  if (info->flow_bank[bank].state != 2)
    {
      info->batch_failed = true;
      leave_critical_section(flags);
      return -EIO;
    }
  info->flow_bank[bank].state = 3; /* CPU owns completed data. */
  *gap = info->flow_bank[bank].gap;
  leave_critical_section(flags);
  up_invalidate_dcache((uintptr_t)info->flow_bank[bank].buffer,
                       (uintptr_t)info->flow_bank[bank].buffer + K7_FLOW_SLOTS * 4096);
  return OK;
}

int k7_xhci_flow_refill(struct usbhost_driver_s *drvr, usbhost_ep_t ep,
                        unsigned int bank)
{
  struct usbhost_xhci_s *priv = XHCI_PRIV_FROM_DRVR(drvr);
  struct xhci_epinfo_s *info = (struct xhci_epinfo_s *)ep;
  int ret;
  if (!info || bank >= K7_FLOW_BANKS || !info->flow_owned || info->batch_failed ||
      info->flow_bank[bank].state != 3 ||
      info->td.i != bank * K7_FLOW_SLOTS) return -EINVAL;
  ret = nxmutex_lock(&priv->lock);
  if (ret < 0) return ret;
  k7_flow_publish(priv, info, bank);
  xhci_ep_doorbell(priv, info);
  nxmutex_unlock(&priv->lock);
  return OK;
}

int k7_xhci_flow_end(usbhost_ep_t ep)
{
  struct xhci_epinfo_s *info = (struct xhci_epinfo_s *)ep;
  int ret = OK;
  if (!info || !info->flow_owned) return -EINVAL;
  if (info->flow_inflight || info->batch_failed)
    {
      info->batch_failed = true;
      ret = -EIO;
    }
  printf("UVC QUEUE refill_gaps=%u ring_empty_events=%u inflight=%u result=%d\n",
         info->flow_refill_gaps, info->flow_empty_events,
         info->flow_inflight, ret);
  info->flow_owned = false;
  /* Keep flow_mode and all banks alive for late completion after timeout. */
  nxmutex_unlock(&info->exclsem);
  return ret;
}


/* Diagnostic split submission: queue 2048 HS microframes in 4 KiB slots.
 * A successful submit MUST be paired with wait by the SAME task. The endpoint
 * mutex remains owned between calls; wait releases it even on timeout.
 * Buffers/results/endpoint stay allocated until reset. A timeout poisons the
 * endpoint, preventing reuse while late DMA completion is possible.
 * The ring still drains between batches; this is not gap-free streaming.
 */
int k7_xhci_isoc_submit(struct usbhost_driver_s *drvr, usbhost_ep_t ep,
                      uint8_t *buffer, size_t packet, int32_t *results)
{
  ...
  ret = nxmutex_lock(&priv->lock);
  if (ret < 0) { nxmutex_unlock(&info->exclsem); return ret; }
  if (info->iocwait || info->batch_active || info->batch_failed || info->flow_mode)
  ...
  xhci_ep_doorbell(priv, info);
  nxmutex_unlock(&priv->lock);
  return OK; /* Paired wait releases exclsem. */
unlock:
  nxmutex_unlock(&priv->lock);
  nxmutex_unlock(&info->exclsem);
  return ret;
...
int k7_xhci_isoc_wait(usbhost_ep_t ep)
{
  struct xhci_epinfo_s *info = (struct xhci_epinfo_s *)ep;
  int ret;

  if (!info || !info->batch_pending_wait) return -EINVAL;
  ret = nxsem_tickwait_uninterruptible(&info->iocsem, SEC2TICK(2));
  if (ret >= 0)
    {
...
int k7_xhci_isoc_batch(struct usbhost_driver_s *drvr, usbhost_ep_t ep,
                     uint8_t *buffer, size_t packet, int32_t *results)
{
  int ret = k7_xhci_isoc_submit(drvr, ep, buffer, packet, results);
  return ret < 0 ? ret : k7_xhci_isoc_wait(ep);
}
...
static int xhci_events_poll(FAR struct usbhost_xhci_s *priv)
{
  ...
  addr = k7_xhci_va_to_pa((FAR void *)addr);
  addr |= XHCI_ERDP_EHB;
  ...
  if (ret >= 0)
    {
      ...
      ctx = k7_xhci_va_to_pa(dev->input);
                        ...
                        FAR usbhost_ep_t *ep)
{
  ...
  if (dev == NULL && !ROOTHUB(hport) &&
    ...
    {
      ret = xhci_ring_init(&epinfo->td, epdesc->xfrtype == USB_EP_ATTR_XFER_ISOC ? 2049 : XHCI_TD_MAX);
      if (ret < 0)
        {
          uerr("ep0 ring init failed\n");
  ...
  xhci_ep_configure(priv, xhci_in_ep(priv, dev->input, idx - 1),
                    eptype, epdesc->mxpacketsize & 0x7ff,
                    hport->speed == USB_SPEED_HIGH &&
  ...
  ret = xhci_cmd_cfgep(priv, epinfo->slot,
                       k7_xhci_va_to_pa(dev->input), false);
  if (ret < 0)
    {
  ...
  for (i = 0; i < priv->no_scratch; i++)
    {
      ...
      memset((FAR void *)(k7_xhci_pa_to_va(priv->pg_sb[i])),
             0, XHCI_PAGE_SIZE);
    }
