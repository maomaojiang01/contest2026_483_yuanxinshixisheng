...
{
 uint32_t v,t,x,mask,value;unsigned i;uint64_t now;
 ...
 if(mod(p,SAI_TXCR,mask,value)||mod(p,SAI_RXCR,mask,value)||
    ...
    mod(p,SAI_PATH_SEL,0xfc0303,0x40000)||
 ...
 if(r->path_active!=((r->path_before&~0xfc0303u)|0x40000u)){rc=-71;goto done;}
 ...
 if(!r->stop_rc&&path_saved){
  r->restore_rc=mod(p,SAI_PATH_SEL,0xfc0303,r->path_before);
  if(!r->restore_rc)r->restore_rc=rd(p,SAI_PATH_SEL,&r->path_restored);
  if(!r->restore_rc&&(r->path_restored&0xfc0303)!=(r->path_before&0xfc0303))r->restore_rc=-71;
 }
