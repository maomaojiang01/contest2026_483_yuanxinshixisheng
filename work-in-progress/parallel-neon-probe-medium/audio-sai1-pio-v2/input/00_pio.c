            ...
            uint32_t *capture,unsigned capacity,struct pio_result*out)
{
  ...
  if(!out)return -22;
  memset(out,0,sizeof *out);
  if(!p || !p->read || !p->write || !p->us || (tx!=0 && tx!=1) ||
      (!tx && (!capture || capacity<2*PIO_FRAMES)))return out->result=-22;
  ...
  start=p->us(p->ctx);
  if(tx) { /* preload one stereo frame while idle */
    if(wr(p,SAI_TXDR,1u<<20)||wr(p,SAI_TXDR,1u<<20))goto done;
    out->frames=1;
  }
  ...
  for(out->polls=0;out->polls<100000;out->polls++) {
    now=p->us(p->ctx);
    if(now<start || now-start>=250000){rc=-110;goto done;}
    ...
    v&=63;
    if(out->frames==PIO_FRAMES) { if(!tx || !v){rc=0;goto done;}continue; }
    if(tx && !v) {
      word=(out->frames/32)%2 ? 0u-(1u<<20) : (1u<<20);
  ...
  if(!out->stop_result)out->held=0;
  out->result=rc?rc:out->stop_result;
  return out->result;
}
