...
paths=[P/s for s in ['work-in-progress/parallel-k7-audio/sources/kernel-6.1/sound/soc/rockchip/rockchip_sai.c','evidence/audio-sai-trm-20260910/SAI-layout.txt','evidence/audio-sai-trm-20260910/input.json','evidence/audio-rxtrace-20260910/k7sound-capture-20260910-194104.bin','evidence/audio-rxtrace-20260910/k7sound-capture-20260910-194104.json','app/k7sound/pio.c']]
records=[]
...
with (R/'test-output.txt').open('wb') as f:
 for opt in ['-O0','-O2']:
  exe='test'+opt[1:]+'.exe'
  for cmd in [[shutil.which('gcc'),'-std=c11',opt,'-Wall','-Wextra','-Werror','-pedantic','reset_probe.c','test.c','-o',exe],[str(R/exe)]]:
