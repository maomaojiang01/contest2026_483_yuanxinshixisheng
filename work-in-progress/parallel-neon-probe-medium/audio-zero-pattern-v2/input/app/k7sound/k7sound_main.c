...
{
  if (argc < 2 || argc > 3) { puts("usage: k7sound probe|capture [frames]|dump|tone [gain_db]|replay [gain_db]; gain_db=0,6,12,18,24"); return 1; }
  ...
  bool maximum = ma4 || hp80 || !strcmp(argv[1], "replay-max") || !strcmp(argv[1], "tone-max");
  ...
  playing = !strcmp(argv[1], "tone") || !strcmp(argv[1], "tone-max") || !strcmp(argv[1], "tone-reset") || !strcmp(argv[1], "tone-clear") || !strcmp(argv[1], "tone-clockwait") || replay;
  bool common = !strcmp(argv[1], "capture-common");
  bool pga24 = !strcmp(argv[1], "capture-pga24");
  ...
  if ((!probe && !playing && !recording) || bus.poisoned || bus.held || data_held || platform.held || reset_fault) goto invalid;
  ...
  if (!rc && !probe)
    {
      ...
      rc = replay ? pio_play_buffer(&pio, 1, 4096000, playback_source, 2 * captured_frames, frames, &result) :
  ...
  int stop_rc = reset_fault ? -EBUSY : platform.ready ? kd_stop(&codec, now_ms(NULL) + 1500) : 0;
