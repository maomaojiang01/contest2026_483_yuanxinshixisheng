...
#include <stdio.h>
#include <string.h>
struct sim {uint32_t reg[32];unsigned fifo,maximum,words,rx,arm,on,off,writes,rx_fifo_reads;uint64_t now;int mode;const uint32_t *source;unsigned limit;};
...
static int rd(void*x,uint32_t o,uint32_t*v){struct sim*s=x;
 ...
 if(o==0x1c){if((s->reg[4]&4)&&s->fifo>=2)s->fifo-=2;*v=encode(s->fifo);
   ...
   return 0;}
 if(o==0x20){s->rx_fifo_reads++;if(s->mode==8&&s->rx_fifo_reads==3)return -1;
   *v=(s->reg[4]&8)&&s->mode!=5?encode(2):0;return 0;}
 if(o==0x34){unsigned n=s->rx++;*v=s->mode==7?(n%8<2?0xffffffffu:0):n;return 0;}
...
int main(void){struct sim s={0};struct pio_result r;static uint32_t data[96000];
 ...
 memset(&s,0,sizeof s);s.mode=7;
 assert(!pio_run(&p,1,4096000,0,data,96000,32,&r));
 assert(r.rx_trace.count==32&&s.rx==64&&r.frames==32);
 assert(r.rx_trace.window_started&&r.rx_trace.end_us>r.rx_trace.start_us);
 assert(r.rx_trace.init[2]==0x00400fff&&r.rx_trace.init[5]==11);
 {unsigned i;for(i=0;i<64;i++)assert(data[i]==(i%8<2?0xffffffffu:0));
  for(i=0;i<32;i++){
   const struct pio_rx_trace_entry *e=&r.rx_trace.entry[i];
   assert(e->word==data[i]&&!e->before_rc&&!e->word_rc&&!e->after_rc);
   assert(e->fifo_before==0x41&&e->fifo_after==0x41);
   if(i)assert(e->time_us>r.rx_trace.entry[i-1].time_us);
  }}
 printf("TRACE simulated count=%u start=%llu end=%llu first_word=%08x third_word=%08x\n",
  r.rx_trace.count,(unsigned long long)r.rx_trace.start_us,(unsigned long long)r.rx_trace.end_us,
  r.rx_trace.entry[0].word,r.rx_trace.entry[2].word);
 memset(&s,0,sizeof s);s.mode=8;
 assert(!pio_run(&p,1,4096000,0,data,96000,32,&r));
 assert(r.rx_trace.entry[0].before_rc==-5&&r.frames==32&&s.rx==64);
 memset(&s,0,sizeof s);assert(!pio_run(&p,1,4096000,0,data,96000,1,&r));
 assert(r.rx_trace.count==2&&s.rx==2);
 puts("PASS trace:32-entry cap,all words including zero retained,registers/time,error validity,existing regressions");return 0;}
