...
Controller::Controller(TtsPort& tts, WifiPort& wifi, Config config)
    : tts_(tts), wifi_(wifi), config_(std::move(config)) {
...
Controller::~Controller() { wipePassword(); }
  ...
  switch (context_.state) {
    case State::WaitingWake:
      if (isWakeWord(text, config_.wake_words)) {
        if (!tts_.speak(config_.greeting)) {
...
bool Controller::handleInputStatus(IoStatus status) {
  switch (status) {
    ...
    case IoStatus::EndOfInput:
      // Input source exhausted: wipe secrets and signal the caller to exit.
      wipePassword();
      return false;
...
void Controller::tryConnect(const char* password, std::size_t password_len) {
  ...
  const bool secured = ap.secured;
  ConnectResult result;
#if defined(__cpp_exceptions) || defined(__EXCEPTIONS)
  try {
    result = wifi_.connect(ssid, password, password_len);
  } catch (...) {
  ...
  if (result.status == ConnectStatus::Ok) {
    setTerminal(State::Completed, "已连接到" + ssid);
    return;
  }
  if (!secured) {
    context_.state = State::WaitingNetworkChoice;
  ...
  context_.grammar = Grammar::PasswordToken;
  context_.password_mode = PasswordMode::Lower;
  if (result.status == ConnectStatus::WrongPassword) {
    repeat("密码错误，请重新输入，默认小写");
    return;
  }
  if (result.status == ConnectStatus::Timeout) {
    repeat("连接超时，请重新输入密码，默认小写");
    return;
  }
  if (result.status == ConnectStatus::NetworkNotFound) {
    repeat("找不到该网络，请重新输入密码或重新选择网络");
    return;
  }
  repeat("连接失败，请重新输入密码，默认小写");
}

void Controller::repeat(const std::string& text) {
  tts_.stop();
  if (!tts_.speak(text)) context_.state = State::Error;
}
