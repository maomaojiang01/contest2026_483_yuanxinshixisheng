...
int main(int argc, char **argv)
{
  ...
  bool cued = !strcmp(argv[1], "capture-cued");
  bool maximum = cued || ma4 || hp80 || !strcmp(argv[1], "replay-max") || !strcmp(argv[1], "tone-max");
  ...
  if (!rc && loopback)
  ...
  else if (!rc && !probe)
    {
      ...
      if (cued)
        {
          playing = true;
          rc = pio_run(&pio, 1, 4096000, true, capture, 96000, 3200, &cue_result);
          playing = false;
          cue_end_us = now_us(NULL);
          data_held = cue_result.held;
          if (!rc && (cue_result.held || cue_result.stop_result ||
                      cue_result.frames != 3200)) rc = -EIO;
        }
      if (!rc) rc = sample16 ? pio_run16(&pio, 1, 4096000, capture, 96000, frames, &result) :
