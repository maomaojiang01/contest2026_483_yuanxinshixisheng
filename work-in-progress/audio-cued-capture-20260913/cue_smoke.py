...
log_path = here / ('cue-smoke-' + time.strftime('%Y%m%d-%H%M%S') + '.log')
with log_path.open('xb') as log, serial.Serial(
    ...
    def command(text, timeout):
        ...
        while time.monotonic() < end:
            ...
            if re.search(rb'nsh>\s*(?:\x1b\[K)?\s*$', data):
                return bytes(data)
            completed = (b'SOUND result=' in data or b'ASR_MODEL result=' in data or b'boot_ret=' in data or b'RADIO bt-host ret=' in data or
                         b'WIFI shared start ret=' in data or b'VOICE native_network=' in data)
    ...
    command('', 5)
    for text in ('k7sound capture-cued 48000',):
        result = command(text, 15)
