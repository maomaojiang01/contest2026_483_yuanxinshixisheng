"""RAM-only loader for the VoiceLink flow firmware; never writes persistent media."""
...
log_path = root / ('voice-flow-' + mode + '-' + stamp + '.log')
...
try:
    if mode == 'enter':
        ...
        manifest = json.loads((root / 'voice-flow-transfer.json').read_text())
        payload = root / 'voice-flow-first-firmware.bin'
        if hashlib.sha256(payload.read_bytes()).hexdigest() != manifest['sha256']:
            raise RuntimeError('payload file hash mismatch')
        with open_port() as port:
            port.write(b'\x03')
            port.flush()
            read(port, 5, rb'(?:^|\n)=>\s*$')
            ...
            copy(port, 0x46C00800, 0x40400000, manifest['firmware_bytes'])
