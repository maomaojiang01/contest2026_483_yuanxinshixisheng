...
import serial
r=Path(__file__).resolve().parent/('network-'+time.strftime('%Y%m%d-%H%M%S'));r.mkdir()
s=serial.Serial(port=None,baudrate=1500000,timeout=.02,write_timeout=2)
s.dtr=False;s.rts=False;s.port='/dev/ttyUSB0';s.open()
rows=[]
try:
    for name,cmd,limit,marker in [('sdio','k7radio sdio-boot',40,b'nsh>'),('ble','k7radio bt-host',35,b'connect=true advertise=0'),('service','k7radio wifi-service-start',15,b'WIFI shared start ret=0'),('scan','k7voice scan',40,b'VOICE scan_done')]:
