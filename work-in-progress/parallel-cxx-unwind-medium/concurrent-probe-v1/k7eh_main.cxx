 ...
 * Explicit two-worker exception diagnostic. No peripheral or model access.
 */
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
...
constexpr unsigned warm_rounds = 64;
...
#ifdef __NuttX__
    s.cpu = sched_getcpu();
    if (s.cpu != int(s.id + 4)) ++s.cpu_mismatches;
#endif
    if (round == 0) s.first_begin = now_ns();
    one_throw(s, round);
    if (round == 0) s.first_end = now_ns();
#ifdef __NuttX__
    if (sched_getcpu() != int(s.id + 4)) ++s.cpu_mismatches;
#endif
  }
...
{
  std::printf("K7EH REBOOT_REQUIRED reason=%s owner_retained=1\n", reason);
  std::fflush(stdout);
...
int create(unsigned i)
{
  ...
  if (destroy_rc) {
    ...
    std::printf("K7EH attr_destroy_error=%d\n", destroy_rc);
    stop.store(1);
  }
  return rc;
}
...
extern "C" int k7eh_main(int argc, char **argv)
{
  if (argc != 2 || (std::strcmp(argv[1], "cold") && std::strcmp(argv[1], "warm"))) {
    std::puts("usage: k7eh cold|warm (each needs a fresh boot; target CPUs 4 and 5)");
    return 1;
  }
  if (attempted.exchange(1)) { std::puts("K7EH already attempted; reboot required"); return 2; }
  const bool warm = !std::strcmp(argv[1], "warm");
  const unsigned rounds = warm ? warm_rounds : 1;
  std::printf("K7EH begin mode=%s rounds=%u workers=2 main_prethrow=%u\n", argv[1], rounds, warm);
  if (warm) {
    Slot preheat;
    preheat.id = 99;
    one_throw(preheat, 0);
    if (preheat.errors || preheat.caught != 1 || preheat.cleaned != 3) return 22;
  }
  unsigned created = 0;
  int result = 0;
  for (unsigned i = 0; i < workers; ++i) {
    ...
    if (rc) {
      std::printf("K7EH create_failed index=%u rc=%d\n", i, rc);
      result = 20;
      break;
    }
    ++created;
  }
  if (created == workers && !stop.load()) {
    ...
    for (unsigned r = 1; r <= rounds && !result; ++r) {
      ...
      if (!begin || !now || now < begin || now - begin >= limit ||
          !reached(workers, r, false, limit - (now - begin))) {
        result = 21;
        break;
      }
      permit.store(r);
  ...
  for (unsigned i = 0; i < created; ++i) {
    ...
    std::printf("K7EH worker=%u caught=%u cleaned=%u errors=%u cpu=%d cpu_mismatch=%u first_begin=%llu first_end=%llu\n",
                i, s.caught, s.cleaned, s.errors, s.cpu, s.cpu_mismatches,
  ...
  std::printf("K7EH result=%s code=%d created=%u joined=%u mode=%s concurrency_proven=0\n",
...
int main(int argc, char **argv)
{
  if (argc == 2 && !std::strcmp(argv[1], "repeat")) {
    ...
    if (k7eh_main(2, args)) return 99;
    return k7eh_main(2, args);
  }
  return k7eh_main(argc, argv);
}
#endif
