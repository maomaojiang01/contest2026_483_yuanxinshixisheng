"""Run existing bounded muted RX2 diagnostic; do not alter microphone data."""
import re,time
from pathlib import Path
import serial
r=Path(__file__).resolve().parent
with (r/('guard-'+time.strftime('%Y%m%d-%H%M%S')+'.log')).open('xb') as log:
    ...
    try:
        ...
        s.write(b'k7sound loopback-guard\r')
        data=read(10)
