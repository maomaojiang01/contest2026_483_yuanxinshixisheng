...
static pthread_mutex_t asr_gate=PTHREAD_MUTEX_INITIALIZER;
static int recognize(bool microphone) {
 if(pthread_mutex_trylock(&asr_gate)){std::puts("ASR_INFER busy");return 1;}
 struct Gate {~Gate(){pthread_mutex_unlock(&asr_gate);}} gate;
 try {
  double start=seconds();Handles h; if(k7_tls_selftest())throw std::runtime_error("TLS isolation check");
  std::unique_ptr<float[]> recorded;
  unsigned frames=test_frames;
  if(microphone) {
   recorded.reset(new float[48000]);
   if(k7sound_copy_mono(recorded.get(),48000,&frames))throw std::runtime_error("no completed microphone recording");
  ...
  for(unsigned offset=0;offset<frames;) {
   unsigned n=frames-offset;if(n>1600)n=1600;
   for(unsigned i=0;i<n;i++){
    if(microphone){samples[i]=recorded[offset+i];continue;}
    unsigned j=2*(offset+i);int value=test_pcm[j]|(unsigned(test_pcm[j+1])<<8);if(value>=32768)value-=65536;samples[i]=float(value)/32768.0f;
   }
...
extern "C" int k7_asr_model_session_probe(){return recognize(false);}
extern "C" int k7_asr_microphone_probe(){return recognize(true);}
