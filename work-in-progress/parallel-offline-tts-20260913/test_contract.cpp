...
extern "C" const SherpaOnnxOfflineTts *SherpaOnnxCreateOfflineTts(
    const SherpaOnnxOfflineTtsConfig *c) {
  assert(c->model.num_threads == 1);
  return mode == 1 ? nullptr : reinterpret_cast<const SherpaOnnxOfflineTts *>(1);
}
extern "C" void SherpaOnnxDestroyOfflineTts(const SherpaOnnxOfflineTts *) {
  ++destroyed;
}
extern "C" void SherpaOnnxDestroyOfflineTtsGeneratedAudio(
    const SherpaOnnxGeneratedAudio *) { ++freed; }
extern "C" const SherpaOnnxGeneratedAudio *
...
int main() {
  ...
  std::string long_text(241, 'a');
  r.text = long_text.c_str();
  ...
  std::puts("PASS: bounded contract and cleanup");
}
