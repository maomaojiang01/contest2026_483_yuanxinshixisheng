...
class Runtime final {
 ...
 fail:failed_bytes=n;++failures;unlock();return nullptr;
 }
 ...
 OrtStatus* init_borrowed(OrtEnv*env){auto*s=init(env);if(!s)own_env_=false;return s;}
 ...
 bool close(){if(leases_)return false;if(!env_)return !fault;
  ...
  if(own_env_){api_->ReleaseEnv(env_);}env_=nullptr;
