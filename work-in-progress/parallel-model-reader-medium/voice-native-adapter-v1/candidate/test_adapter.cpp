...
struct Fixture {
 k7wd_dispatch d{}; ws_service s{}; std::mutex mutex; unsigned depth=0, scans=0,stops=0,joins=0;
 ...
 static int scan(void*p,uint64_t id){auto*f=(Fixture*)p;C(!f->depth);f->scans++;f->id=id;C(k7wd_pump(&f->d)==WD_BUSY);return f->reject;}
 static int stop(void*p,uint64_t id){auto*f=(Fixture*)p;C(!f->depth&&f->id==id);f->stops++;return f->stop_fail;}
 Fixture(bool offline=true){k7wd_port dp{this,lock,unlock,clock,join,stop_join};C(k7wd_init(&d,&dp,offline)==0);ws_port sp{this,scan,stop};C(ws_attach(&s,&d,&sp)==0);}
 void pump(){C(k7wd_pump(&d)==WD_OK);}
 ws_done done(uint64_t t,uint64_t seq=1){return {t,seq,0,true,true,true,true,true,true,true};}
...
static void ready_and_busy(){
 ...
 uint64_t join;C(k7wd_ble_begin(&f.d,"AP1",3,"12345678",8,500,&join)==WD_OK);f.pump();k7wd_view v{};
 C(k7wd_ble_poll(&f.d,join,&v)==WD_OK&&v.status==WB_BUSY&&f.joins==0);
...
static void connection(){
 ...
 k7wd_event e{};e.kind=WD_PROGRESS;e.done={f.job.owner,f.job.id,1,1,1,1,1,1,{10,3,0,214}};
 C(k7wd_post(&f.d,&e)==WD_OK);f.pump();C(p.pollConnect(a.request_id).status==ConnectStatus::Connecting);
 e.kind=WD_COMPLETION;e.done.sequence=2;C(k7wd_post(&f.d,&e)==WD_OK);f.pump();auto ip=p.pollConnect(a.request_id);C(ip.status==ConnectStatus::IpReady&&ip.ipv4=="10.3.0.214");
 scan=p.beginScan();f.pump();C(p.pollScan(scan.request_id).status==ScanStatus::Unsupported&&f.scans==0&&f.d.broker.radio==WB_OWNED);
 C(k7wd_ble_release(&f.d,a.request_id)==WD_FORBIDDEN);C(k7wd_voice_release(&f.d,a.request_id)==WD_OK);f.pump();
 e.done.sequence=3;e.done.link_up=0;e.done.success=0;C(k7wd_post(&f.d,&e)==WD_OK);f.pump();C(f.d.broker.radio==WB_FREE);
 scan=p.beginScan();f.pump();e.done.sequence=4;C(k7wd_post(&f.d,&e)==WD_OK);f.pump();C(f.d.broker.radio==WB_SCAN);f.finish(scan.request_id);
}
static void races(){
 for(unsigned round=0;round<100;round++) {
  Fixture f;uint64_t a=0,b=0;k7wd_rc ra,rb;
...
static void invalid_ip_and_text(){
 for(unsigned bad=0;bad<5;bad++) {
  ...
  k7wd_event e{};e.kind=WD_COMPLETION;e.done={f.job.owner,f.job.id,1,1,1,1,1,1,{10,3,0,214}};
  ...
  C(k7wd_post(&f.d,&e)==WD_OK);f.pump();C(p.pollConnect(a.request_id).status!=ConnectStatus::IpReady&&f.d.broker.radio==WB_DRAINING);
 }
 for(unsigned bad=0;bad<3;bad++) {
  Fixture f;SharedWifiPort p(f.d,f.s);auto a=p.beginScan();f.pump();auto r=f.record();r.ssid[1]=bad==0?0:bad==1?0xff:0xc0;
  C(ws_record(&f.s,a.request_id,&r)==SC_OK);f.finish(a.request_id);C(p.pollScan(a.request_id).status==ScanStatus::Ready&&p.pollScan(a.request_id).access_points.empty());
 }
}
static void mixed_view_and_error(){
 Fixture f;SharedWifiPort p(f.d,f.s);auto a=p.beginScan();f.pump();auto bad=f.record(1);bad.ssid[0]=0xff;auto good=f.record(2);
 C(ws_record(&f.s,a.request_id,&bad)==SC_OK);C(ws_record(&f.s,a.request_id,&good)==SC_OK);f.finish(a.request_id);
 auto v=p.pollScan(a.request_id);C(v.status==ScanStatus::Ready&&v.access_points.size()==1&&v.access_points[0].ssid=="AP1");C(f.s.view.count==2&&f.s.view.ap[0].ssid[0]==0xff);
 auto b=p.beginScan();f.pump();auto e=f.done(b.request_id);e.success=false;e.error=-123;C(ws_post(&f.s,&e)==WD_OK);f.pump();C(p.pollScan(b.request_id).status==ScanStatus::Failed&&p.pollScan(b.request_id).error_code==-123);
}
static bool zero_bytes(const void*p,size_t n){const unsigned char*b=(const unsigned char*)p;while(n--)if(*b++)return false;return true;}
...
int main(){native_and_credentials();mixed_view_and_error();races();invalid_ip_and_text();ready_and_busy();fences();boundaries();connection();std::printf("PASS checks=%u; real candidate functions, injected mock hardware only; no radio/IP acceptance\n",checks.load());}
