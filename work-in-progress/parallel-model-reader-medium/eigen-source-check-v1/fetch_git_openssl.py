...
for cmd in [['git','init',str(p/'git-proof')],['git','-C',str(p/'git-proof'),'-c','credential.helper=','-c','http.sslBackend=openssl','fetch','--depth=1','--no-tags','https://gitlab.com/libeigen/eigen.git','e7248b26a1ed53fa030c5c459f7ea095dfd276ac']]:
 try:
...
(p/'git-fetch-openssl.json').write_text(json.dumps(log,indent=2));print(json.dumps(log,indent=2))
