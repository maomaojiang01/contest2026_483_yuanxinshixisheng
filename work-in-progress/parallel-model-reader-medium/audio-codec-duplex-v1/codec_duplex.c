...
static const struct item init[] = {
 ...
 {0x06,0xc3,0,0},{0x19,0x06,0x04,0},{0x0f,0x34,0x04,0},
...
static int finish(struct kd_codec*c,int e){
 if(e){c->error=e;c->state=KD_FAULT;}
...
int kd_prepare(struct kd_codec*c,unsigned bits,uint64_t d){
 int e;if(!c || (bits!=16 && bits!=32))return -EINVAL;if(c->busy)return -EBUSY;if(c->state!=KD_OFF)return -EBUSY;
