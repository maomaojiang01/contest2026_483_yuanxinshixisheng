...
/* Capture-only request experiment. No DMA engine access/submission. Caller
 * must ensure no DMA engine channel can respond to SAI RX request. */
struct pio_rx_request_report {
 uint32_t initial_dmacr,enabled_dmacr,final_dmacr,slot_masks[8];
 int initial_rc,enabled_rc,final_rc,mask_rc[8],disable_rc;
 unsigned initial_seen,enable_attempted,disable_attempted;
};
int pio_run_rx_request(const struct pio_port *,int,uint32_t,uint32_t *,
 unsigned,unsigned,struct pio_result *,struct pio_rx_request_report *);
#endif
