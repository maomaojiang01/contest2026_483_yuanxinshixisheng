...
try:
 ...
 run([CC.with_name('nm.exe'),'-u',E/'diagnostic.o'])
 assert 'ggml_backend_alloc_ctx_tensors_from_buft' not in records[-1]['stdout']
 assert 'realloc' not in records[-1]['stdout']
 assert 'ggml_backend_tensor_alloc' in records[-1]['stdout']
