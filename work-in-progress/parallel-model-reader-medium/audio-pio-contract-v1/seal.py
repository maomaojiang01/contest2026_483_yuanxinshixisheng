...
(p/'outputs.json').write_text(json.dumps(a,indent=2));(p/'delivery.json').write_text(json.dumps({'status':'READ_ONLY_NO_PIO_BANK_CONTRACT_FOUND','outputs_sha256':h(p/'outputs.json'),'inputs_sha256':h(p/'inputs.json')},indent=2))
for x in a:assert h(p/x['path'])==x['sha256']
for n in ['delivery.json','outputs.json']:print(n,h(p/n))
