...
(p/'outputs.json').write_text(json.dumps(files,indent=2));d={'status':'CMAKE_CANDIDATE_NATIVE_CONFIGURE_BUILD_PENDING','outputs_sha256':hashlib.sha256((p/'outputs.json').read_bytes()).hexdigest(),'cmake_sha256':files['session-initial-cache.cmake']['sha256'],'scope':'CPU Add Session build map, not full model operators'};(p/'delivery.json').write_text(json.dumps(d,indent=2));print(json.dumps(d));print('delivery_sha256='+hashlib.sha256((p/'delivery.json').read_bytes()).hexdigest())
