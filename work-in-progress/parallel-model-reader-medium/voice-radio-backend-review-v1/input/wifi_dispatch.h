...
enum wd_rc { WD_OK,WD_FULL,WD_INVALID,WD_UNKNOWN,WD_FORBIDDEN,WD_BUSY,WD_EXHAUSTED };
enum wd_phase { WD_QUEUED,WD_BROKER };
enum wd_event_kind { WD_PROGRESS,WD_COMPLETION };
struct wd_view { uint64_t ticket;enum wd_phase phase;enum wb_status status;bool held;uint8_t ip[4]; };
struct wd_event { enum wd_event_kind kind;struct wb_done done; };
struct wd_port {
 void *ctx;
...
struct wd_command {uint64_t ticket,owner,submitted,timeout;struct wb_job credentials;};
struct wd_slot {uint64_t owner;struct wd_view view;bool live,cancel,release;};
struct wd_dispatch {
 ...
 struct wd_command requests[WD_REQUESTS];unsigned rq_head,rq_count;
 struct wd_event events[WD_EVENTS];unsigned ev_head,ev_count;
 struct wd_slot slots[WD_RESULTS];uint64_t next_ticket;
 bool pumping;
...
int wd_init(struct wd_dispatch *,const struct wd_port *,bool proven_offline);
...
enum wd_rc wd_ble_begin(struct wd_dispatch *,const char *,size_t,const char *,size_t,uint64_t,uint64_t *);
enum wd_rc wd_voice_begin(struct wd_dispatch *,const char *,size_t,const char *,size_t,uint64_t,uint64_t *);
enum wd_rc wd_ble_poll(struct wd_dispatch *,uint64_t,struct wd_view *);
enum wd_rc wd_voice_poll(struct wd_dispatch *,uint64_t,struct wd_view *);
enum wd_rc wd_ble_cancel(struct wd_dispatch *,uint64_t);
enum wd_rc wd_voice_cancel(struct wd_dispatch *,uint64_t);
enum wd_rc wd_ble_release(struct wd_dispatch *,uint64_t);
enum wd_rc wd_voice_release(struct wd_dispatch *,uint64_t);
 ...
 */
enum wd_rc wd_post(struct wd_dispatch *,const struct wd_event *);
...
enum wd_rc wd_pump(struct wd_dispatch *);
#endif
