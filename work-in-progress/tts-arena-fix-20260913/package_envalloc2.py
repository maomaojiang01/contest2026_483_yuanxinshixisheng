...
BUILD = Path('/home/swl/openvela/cmake_out/velavision_spoken_tts_gated_envalloc2_20260914')
...
result = json.loads((HERE / 'envalloc2-firmware-result.json').read_text())
...
(HERE / 'spoken-tts-gated-envalloc2-prefix.bin').write_bytes(payload)
record = {
    'bytes': len(payload),
    'sha256': sha(payload),
...
(HERE / 'envalloc2-prefix-package.json').write_text(
