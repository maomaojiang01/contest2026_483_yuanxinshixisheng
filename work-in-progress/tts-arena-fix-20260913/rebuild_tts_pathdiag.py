...
replacement = HERE / 'offline-tts-vits-model-pathdiag.cc.o'
...
run(command, 'pathdiag-compile-tts-model.log')
...
original = HERE / 'offline-tts-vits-model-pathdiag.original.o'
...
weakened = HERE / 'speech-runtime-fixed-tts-weakened-pathdiag.o'
shutil.copyfile(CURRENT, weakened)
run([objcopy, *['--weaken-symbol=' + s for s in symbols], str(weakened)],
    'pathdiag-weaken-tts-model.log')
fixed = HERE / 'speech-runtime-pathdiag-base.arm64.o'
run([ld, '-r', '--strip-debug', '-o', str(fixed), str(weakened),
     str(replacement)], 'pathdiag-link-tts-model.log')
...
if result['tts_entry_count'] != 1:
    result['status'] = 'fail'
(HERE / 'pathdiag-base-result.json').write_text(json.dumps(result, indent=2) + '\n')
