...
BUILD = SDK / 'cmake_out/velavision_spoken_tts_gated_speechcache_20260914'
TOOLS = SDK / 'prebuilts/gcc/linux-x86_64/aarch64-none-elf/bin'
RUNTIME = HERE / 'speech-runtime-gated-speechcache.arm64.o'
PACKAGE = HERE / 'spoken-tts-gated-speechcache-prefix.bin'
...
runtime_result = json.loads((HERE / 'speechcache-runtime-result.json').read_text())
firmware_result = json.loads((HERE / 'speechcache-firmware-result.json').read_text())
package_result = json.loads((HERE / 'speechcache-prefix-package.json').read_text())
...
(HERE / 'speechcache-package-verification.json').write_text(
