...
BUILD = Path('/home/swl/openvela/cmake_out/velavision_spoken_tts_gated_speechcache_20260914')
...
result = json.loads((HERE / 'speechcache-firmware-result.json').read_text())
...
(HERE / 'spoken-tts-gated-speechcache-prefix.bin').write_bytes(payload)
record = {
    'bytes': len(payload),
    'sha256': sha(payload),
...
(HERE / 'speechcache-prefix-package.json').write_text(
