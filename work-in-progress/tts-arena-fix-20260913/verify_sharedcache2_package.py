...
BUILD = SDK / 'cmake_out/velavision_spoken_tts_gated_sharedcache2_20260914'
TOOLS = SDK / 'prebuilts/gcc/linux-x86_64/aarch64-none-elf/bin'
RUNTIME = HERE / 'speech-runtime-gated-sharedcache2.arm64.o'
PACKAGE = HERE / 'spoken-tts-gated-sharedcache2-prefix.bin'
...
EXPECTED_STORAGE_SHA256 = '5e3a76b1fcc31f198072cf46dbf2377f81a85242c6f7c2adbc9f31112e90ed02'
EXPECTED_SOUND_SHA256 = '48eb02cee4f5dd21bd9c79a92de158361c494c51ac97041f807f8e73ad8d2aac'
...
runtime_result = json.loads((HERE / 'sharedcache2-runtime-result.json').read_text())
firmware_result = json.loads((HERE / 'sharedcache2-firmware-result.json').read_text())
package_result = json.loads((HERE / 'sharedcache2-prefix-package.json').read_text())
...
checks = {
    ...
    'shared_allocator_linked': b'SR_ATTACH registered' in runtime_bytes and
                               b'_ZN9sr_shared20shared_model_runtimeEv' in elf_nm.encode(),
    'shared_impl_hash': sha(SHARED_IMPL) ==
                        '144e4db2195bd797bb68a95c1c14aa99da723f2fce8d9a8c355b840784d2f809',
    ...
    'speech_gain_manifest': package_result['speech_gain_db'] == 24,
...
(HERE / 'sharedcache2-package-verification.json').write_text(
