...
replacement = HERE / 'offline-tts-vits-model-flowfix.cc.o'
...
run(command, 'flowfix-compile-tts-model.log')
...
original = HERE / 'offline-tts-vits-model-flowfix.original.o'
...
weakened = HERE / 'speech-runtime-fixed-tts-weakened-flowfix.o'
shutil.copyfile(CURRENT, weakened)
run([objcopy, *['--weaken-symbol=' + s for s in symbols], str(weakened)],
    'flowfix-weaken-tts-model.log')
fixed = HERE / 'speech-runtime-flowfix-base.arm64.o'
run([ld, '-r', '--strip-debug', '-o', str(fixed), str(replacement),
     str(weakened)], 'flowfix-link-tts-model.log')
...
if result['tts_entry_count'] != 1:
    result['status'] = 'fail'
(HERE / 'flowfix-base-result.json').write_text(json.dumps(result, indent=2) + '\n')
