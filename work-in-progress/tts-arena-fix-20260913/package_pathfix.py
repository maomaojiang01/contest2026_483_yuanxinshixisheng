...
BUILD = Path('/home/swl/openvela/cmake_out/velavision_spoken_tts_gated_pathfix_20260914')
...
result = json.loads((HERE / 'pathfix-firmware-result.json').read_text())
...
(HERE / 'spoken-tts-gated-pathfix-prefix.bin').write_bytes(payload)
record = {
    'bytes': len(payload),
    'sha256': sha(payload),
...
(HERE / 'pathfix-prefix-package.json').write_text(
