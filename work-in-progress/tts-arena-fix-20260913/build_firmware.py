...
BUILD = SDK / 'cmake_out/velavision_spoken_tts_arena_fix_20260913'
RUNTIME = HERE / 'tts-with-current-asr-fixed.arm64.o'
EXPECTED_RUNTIME = '0f26d8fed6c713a976fa834c1e1a3170dc8ec488f5baf9000088898468540c22'
...
if sha(RUNTIME) != EXPECTED_RUNTIME:
    raise RuntimeError('fixed runtime hash changed')
if BUILD.exists():
...
environment['PATH'] = ':'.join(str(SDK / p) for p in [
...
record = {
    ...
    'inputs': prior['inputs'], 'board_tested': False,
}
for phase, command in [('configure', configure),
    ...
    (HERE / 'firmware-result.json').write_text(json.dumps(record, indent=2) + '\n')
