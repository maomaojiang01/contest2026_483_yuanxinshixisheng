...
CURRENT = HERE / 'speech-runtime-fixed.arm64.o'
...
replacement = HERE / 'offline-tts-vits-model-envalloc2.cc.o'
...
run(command, 'envalloc2-compile-tts-model.log')
...
original = HERE / 'offline-tts-vits-model-envalloc2.original.o'
...
weakened = HERE / 'speech-runtime-fixed-tts-weakened-envalloc2.o'
shutil.copyfile(CURRENT, weakened)
run([objcopy, *['--weaken-symbol=' + s for s in symbols], str(weakened)],
    'envalloc2-weaken-tts-model.log')
fixed = HERE / 'speech-runtime-envalloc2-base.arm64.o'
run([ld, '-r', '--strip-debug', '-o', str(fixed), str(replacement),
     str(weakened)], 'envalloc2-link-tts-model.log')
...
if result['tts_entry_count'] != 1:
    result['status'] = 'fail'
(HERE / 'envalloc2-base-result.json').write_text(json.dumps(result, indent=2) + '\n')
