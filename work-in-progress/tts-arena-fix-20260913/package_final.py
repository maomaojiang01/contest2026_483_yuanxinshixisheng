...
BUILD = Path('/home/swl/openvela/cmake_out/velavision_spoken_tts_asr_arena_fix_20260913')
...
result = json.loads((HERE / 'final-firmware-result.json').read_text())
...
(HERE / 'spoken-tts-asr-fixed-prefix.bin').write_bytes(payload)
record = {
    'bytes': len(payload),
    'sha256': sha(payload),
...
(HERE / 'final-prefix-package.json').write_text(
