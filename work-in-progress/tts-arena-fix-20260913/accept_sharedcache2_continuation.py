"""Continue sharedcache2 acceptance after the already-recorded cold TTS."""
...
LOG_PATH = HERE / ("sharedcache2-continuation-" + STAMP + ".log")
REPORT_PATH = HERE / ("sharedcache2-continuation-" + STAMP + ".json")
...
def tts_metrics(output):
    ...
    frames, rate, peak, clipped, cache, seconds, _, _ = one(
        ...
        output, "TTS audio")
    if hit != b"1" or cache != b"hit":
        raise RuntimeError("TTS cache was not reused")
    if speakers != b"174" or sid != b"21" or rate != b"8000":
        raise RuntimeError("unexpected TTS voice metadata")
    if int(frames) <= 0 or int(peak) <= 0 or clipped != b"0":
        raise RuntimeError("invalid TTS PCM diagnostics")
    if b"SOUND playback_gain_db=24 dac_attenuation_db=0 maximum=0" not in output:
        raise RuntimeError("speech gain was not +24 dB")
    ...
    return {"hit": 1, "init_seconds": float(initialized),
            "total_seconds": float(seconds), "frames": int(frames),
            "peak_milli": int(peak), "speaker_id": 21}
...
with LOG_PATH.open("xb") as log, serial.Serial(
    ...
    command("", 5)
    free_after_tts = available(command("k7mem status", 10))
    first_tts_warm = tts_metrics(command("k7voice tts-smoke", 240))
    first_asr = asr_metrics(command("k7voice asr-model", 240), 0)
    free_after_asr = available(command("k7mem status", 10))
    second_asr = asr_metrics(command("k7voice asr-model", 240), 1)
    second_tts_warm = tts_metrics(command("k7voice tts-smoke", 240))
    free_final = available(command("k7mem status", 10))

if first_tts_warm["init_seconds"] >= .5 or second_asr["init_seconds"] >= .5:
...
if free_after_asr <= 0 or free_final <= 0:
    raise RuntimeError("model arena exhausted")

report = {
    "status": "pass",
    "sequence": ["tts-warm", "asr-cold", "asr-warm", "tts-warm"],
    "tts": [first_tts_warm, second_tts_warm],
    "asr": [first_asr, second_asr],
    "arena_free": {"after_tts": free_after_tts,
                   "after_asr": free_after_asr, "final": free_final},
