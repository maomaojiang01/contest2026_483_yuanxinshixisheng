...
BUILD = Path('/home/swl/openvela/cmake_out/velavision_spoken_tts_gated_flowui_20260914')
...
result = json.loads((HERE / 'flowui-firmware-result.json').read_text())
...
(HERE / 'spoken-tts-gated-flowui-prefix.bin').write_bytes(payload)
record = {
    'bytes': len(payload),
    'sha256': sha(payload),
...
(HERE / 'flowui-prefix-package.json').write_text(
