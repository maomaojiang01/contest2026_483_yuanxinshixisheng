...
BUILD = Path('/home/swl/openvela/cmake_out/velavision_spoken_tts_arena_fix_20260913')
...
result = json.loads((HERE / 'firmware-result.json').read_text())
...
(HERE / 'spoken-tts-prefix.bin').write_bytes(payload)
record = {
    'bytes': len(payload), 'sha256': sha(payload),
    'crc32': f'{zlib.crc32(payload) & 0xffffffff:08x}',
    'firmware_bytes': len(firmware), 'firmware_sha256': sha(firmware),
    'firmware_crc32': f'{zlib.crc32(firmware) & 0xffffffff:08x}',
    'flash_commands': False,
}
(HERE / 'prefix-package.json').write_text(json.dumps(record, indent=2) + '\n')
