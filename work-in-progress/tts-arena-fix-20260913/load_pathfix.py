...
environment['K7VOICE_MANIFEST'] = str(HERE / 'pathfix-prefix-package.json')
environment['K7VOICE_PAYLOAD'] = str(HERE / 'spoken-tts-gated-pathfix-prefix.bin')

if len(sys.argv) != 2 or sys.argv[1] not in ('enter', 'transfer'):
    raise SystemExit('usage: load_pathfix.py enter|transfer')
if sys.argv[1] == 'enter':
