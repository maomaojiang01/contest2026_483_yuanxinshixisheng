...
BUILD = Path('/home/swl/openvela/cmake_out/velavision_spoken_tts_gated_20260913')
...
result = json.loads((HERE / 'gated-firmware-result.json').read_text())
...
(HERE / 'spoken-tts-gated-prefix.bin').write_bytes(payload)
record = {
    'bytes': len(payload),
    'sha256': sha(payload),
...
(HERE / 'gated-prefix-package.json').write_text(
