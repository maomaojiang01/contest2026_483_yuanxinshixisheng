...
run(command, 'shared-compile-asr.log')
...
run([objcopy, *['--weaken-symbol=' + item for item in symbols], str(weakened)],
    'shared-weaken-asr.log')
fixed = HERE / 'speech-runtime-shared-asr.arm64.o'
run([ld, '-r', '--strip-debug', '-o', str(fixed), str(weakened),
     str(replacement)], 'shared-link-asr.log')
...
if result['asr_constructor_count'] != 1 or result['tts_entry_count'] != 1:
    result['status'] = 'fail'
(HERE / 'shared-asr-result.json').write_text(
