...
BUILD = SDK / 'cmake_out/velavision_spoken_tts_gated_flowui2_20260914'
TOOLS = SDK / 'prebuilts/gcc/linux-x86_64/aarch64-none-elf/bin'
RUNTIME = HERE / 'speech-runtime-gated-flowfix.arm64.o'
PACKAGE = HERE / 'spoken-tts-gated-flowui2-prefix.bin'
...
EXPECTED_STORAGE_SHA256 = '5ec0307f047550edfe7c925b73d5ef106a24598527922bf8b2ccc1ee6acc2594'
EXPECTED_SOUND_SHA256 = '77b6b9d657720cd916a4083540e39577dbc92b2a14f47a95d0e8e39a4564c773'
...
runtime_result = json.loads((HERE / 'flowfix-runtime-result.json').read_text())
firmware_result = json.loads((HERE / 'flowui2-firmware-result.json').read_text())
package_result = json.loads((HERE / 'flowui2-prefix-package.json').read_text())
...
checks = {
    ...
    'tts_asr_capacity_match': '#define SR_CAPACITY 2048' in
                              STORAGE_SOURCE.read_text() and
                              '#define SR_CAPACITY 2048' in
...
(HERE / 'flowui2-package-verification.json').write_text(
