...
BUILD = SDK / 'cmake_out/velavision_spoken_tts_gated_20260913'
TOOLS = SDK / 'prebuilts/gcc/linux-x86_64/aarch64-none-elf/bin'
RUNTIME = HERE / 'speech-runtime-gated.arm64.o'
PACKAGE = HERE / 'spoken-tts-gated-prefix.bin'
...
runtime_result = json.loads((HERE / 'gated-runtime-result.json').read_text())
firmware_result = json.loads((HERE / 'gated-firmware-result.json').read_text())
package_result = json.loads((HERE / 'gated-prefix-package.json').read_text())
...
(HERE / 'gated-package-verification.json').write_text(
