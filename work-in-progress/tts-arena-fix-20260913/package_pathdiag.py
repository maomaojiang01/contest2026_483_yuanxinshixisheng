...
BUILD = Path('/home/swl/openvela/cmake_out/velavision_spoken_tts_gated_pathdiag_20260914')
...
result = json.loads((HERE / 'pathdiag-firmware-result.json').read_text())
...
(HERE / 'spoken-tts-gated-pathdiag-prefix.bin').write_bytes(payload)
record = {
    'bytes': len(payload),
    'sha256': sha(payload),
...
(HERE / 'pathdiag-prefix-package.json').write_text(
