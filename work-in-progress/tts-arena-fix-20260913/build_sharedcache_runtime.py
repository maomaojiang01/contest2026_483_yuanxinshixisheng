...
BASE = HERE / 'speech-runtime-shared-base.arm64.o'
...
for name, tts in [('native_asr_runtime.cpp', False),
    ...
    if tts:
        command = ['-DSHERPA_ONNX_ENABLE_TTS=1'
                   ...
                   for item in command]
    run(command, 'sharedcache-compile-' + name + '.log')
...
base_undefined = sorted(set(line.split()[-1] for line in
    ...
    if line.split()))
weakened = HERE / 'speech-runtime-wrappers-sharedcache-weakened.o'
shutil.copyfile(BASE, weakened)
run([objcopy, *['--weaken-symbol=' + item for item in symbols], str(weakened)],
    'sharedcache-weaken-wrappers.log')
fixed = HERE / 'speech-runtime-gated-sharedcache.arm64.o'
run([ld, '-r', '--strip-debug', '-o', str(fixed), str(weakened),
     *map(str, objects)], 'link-sharedcache.log')
...
if result['added_undefined'] or any(value != 1 for value in
    ...
    result['status'] = 'fail'
(HERE / 'sharedcache-runtime-result.json').write_text(
