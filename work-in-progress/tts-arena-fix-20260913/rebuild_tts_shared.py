...
CURRENT = HERE / 'speech-runtime-shared-asr.arm64.o'
...
replacement = HERE / 'offline-tts-vits-model-shared.cc.o'
...
run(command, 'shared-compile-tts-model.log')
...
original = HERE / 'offline-tts-vits-model-shared.original.o'
...
weakened = HERE / 'speech-runtime-shared-asr-tts-weakened.o'
shutil.copyfile(CURRENT, weakened)
run([objcopy, *['--weaken-symbol=' + s for s in symbols], str(weakened)],
    'shared-weaken-tts-model.log')
fixed = HERE / 'speech-runtime-shared-base.arm64.o'
run([ld, '-r', '--strip-debug', '-o', str(fixed), str(replacement),
     str(weakened)], 'shared-link-tts-model.log')
...
if result['tts_entry_count'] != 1:
    result['status'] = 'fail'
(HERE / 'shared-base-result.json').write_text(json.dumps(result, indent=2) + '\n')
