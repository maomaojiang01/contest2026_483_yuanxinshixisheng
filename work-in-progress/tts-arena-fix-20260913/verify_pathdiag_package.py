...
BUILD = SDK / 'cmake_out/velavision_spoken_tts_gated_pathdiag_20260914'
TOOLS = SDK / 'prebuilts/gcc/linux-x86_64/aarch64-none-elf/bin'
RUNTIME = HERE / 'speech-runtime-gated-pathdiag.arm64.o'
PACKAGE = HERE / 'spoken-tts-gated-pathdiag-prefix.bin'
...
runtime_result = json.loads((HERE / 'pathdiag-runtime-result.json').read_text())
firmware_result = json.loads((HERE / 'pathdiag-firmware-result.json').read_text())
package_result = json.loads((HERE / 'pathdiag-prefix-package.json').read_text())
...
(HERE / 'pathdiag-package-verification.json').write_text(
