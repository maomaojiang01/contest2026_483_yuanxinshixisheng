...
replacement = HERE / 'offline-tts-vits-model-envalloc.cc.o'
...
run(command, 'envalloc-compile-tts-model.log')
...
original = HERE / 'offline-tts-vits-model-envalloc.original.o'
...
weakened = HERE / 'speech-runtime-fixed-tts-weakened-envalloc.o'
shutil.copyfile(CURRENT, weakened)
run([objcopy, *['--weaken-symbol=' + s for s in symbols], str(weakened)],
    'envalloc-weaken-tts-model.log')
fixed = HERE / 'speech-runtime-envalloc-base.arm64.o'
run([ld, '-r', '--strip-debug', '-o', str(fixed), str(weakened),
     str(replacement)], 'envalloc-link-tts-model.log')
...
if result['tts_entry_count'] != 1:
    result['status'] = 'fail'
(HERE / 'envalloc-base-result.json').write_text(json.dumps(result, indent=2) + '\n')
