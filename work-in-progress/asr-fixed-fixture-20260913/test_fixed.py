...
log_path = here / ('fixed-model-' + time.strftime('%Y%m%d-%H%M%S') + '.log')
with log_path.open('xb') as log, serial.Serial(
    ...
    def command(text, timeout):
        ...
        while time.monotonic() < end:
            ...
            if re.search(rb'nsh>\s*(?:\x1b\[K)?\s*$', data):
                return bytes(data)
            completed = (b'ASR_MODEL result=' in data or b'boot_ret=' in data or b'RADIO bt-host ret=' in data or
    ...
    command('', 5)
    for text in ('k7voice asr-model',):
        result = command(text, 210)
