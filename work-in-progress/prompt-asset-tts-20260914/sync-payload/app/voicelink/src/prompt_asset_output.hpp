...
class PromptAssetOutput {
 ...
 private:
  ...
  static const char* classify(const std::string& text) noexcept {
    ...
    if (has(text, "没有发现网络")) return "no_network";
    if (has(text, "编号无效")) return "not_heard";
    if (starts(text, "已选择开放网络")) return "selected";
    if (starts(text, "已选择")) return "selected";
    if (has(text, "没有听清")) return "not_heard";
    if (starts(text, "已输入第") || starts(text, "当前长度为")) return "heard_char";
    ...
    if (has(text, "切换到") || has(text, "切换输入")) return "heard_char";
    ...
    if (has(text, "密码错误")) return "wrong_password";
    if (has(text, "超时")) return "failed";
    if (has(text, "已取消")) return "cancelled";
