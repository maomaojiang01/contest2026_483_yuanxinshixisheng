...
#include <stdio.h>
#include <string.h>
struct sim {uint32_t reg[32];unsigned fifo,maximum,words,rx,arm,on,off,writes;uint64_t now;int mode;const uint32_t *source;const int16_t *source16;unsigned limit;};
...
static int wr(void*x,uint32_t o,uint32_t v){struct sim*s=x;s->writes++;
 if(o==0x30){assert(s->arm && s->fifo<16);s->fifo++;s->words++;
   ...
   else if(s->source16){assert(s->words<=2*s->limit);assert(v==(uint32_t)((int32_t)s->source16[(s->words-1)/2]*65536));}
...
int main(void){struct sim s={0};struct pio_result r;static uint32_t data[96000];
 ...
 for(i=0;i<96000;i++)assert(data[i]==0x9e3779b9u*i+0x80000000u);
  {
   static int16_t mono[48000];
   for(i=0;i<48000;i++)mono[i]=(int16_t)(i*29u-32768u);
   for(j=0;j<sizeof lengths/sizeof lengths[0];j++){
    unsigned n=lengths[j];memset(&s,0,sizeof s);s.source16=mono;s.limit=n;
    assert(!pio_play_mono16(&p,1,4096000,mono,48000,n,&r));
    assert(s.words==2*n&&r.frames==n&&s.maximum<=16&&!r.held);
   }
   memset(&s,0,sizeof s);
   assert(pio_play_mono16(&p,1,4096000,NULL,48000,1,&r)==-22);
   assert(pio_play_mono16(&p,1,4096000,mono,0,1,&r)==-22);
   assert(pio_play_mono16(&p,1,4096000,mono,47999,48000,&r)==-22);
   assert(pio_play_mono16(&p,1,4096000,mono,48000,0,&r)==-22);
   assert(pio_play_mono16(&p,1,4096000,mono,48000,480001,&r)==-22);
  }
  memset(&s,0,sizeof s);
