...
cmake=R.parent/'parallel-llama-b0/tools/python/cmake/data/bin/cmake.exe'
...
try:
    ...
    run('compile',[cmake,'--build',out/'build','--target','pool-test','--parallel','4'],180)
    run('pool-test',[out/'build/pool-test.exe'],30)
finally:
    (out/'inputs.json').write_text(json.dumps([dict(path=str(p.relative_to(R)),sha256=hashlib.sha256(p.read_bytes()).hexdigest())
        for p in [R/'CMakeLists.txt',R/'tests/pool.c',R/'scripts/build.py',R/'include/ggml-pool-safe.h',R/'candidate/ggml-cpu.c']],indent=2),encoding='utf8')
print(out)
