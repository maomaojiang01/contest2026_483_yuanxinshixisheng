...
before = json.loads((HERE / 'before-prefix.json').read_text(encoding='utf-8-sig'))
with (HERE / 'sdk-prefix-audit.log').open('xb') as log:
...
(HERE / 'sync-prefix-result.json').write_text(json.dumps({
