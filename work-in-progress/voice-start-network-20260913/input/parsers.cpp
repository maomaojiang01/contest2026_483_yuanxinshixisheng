...
bool isWakeWord(const std::string& text, const std::vector<std::string>& configured) {
  ...
  // The locked on-device Paraformer model repeatedly renders the contest
  // phrase "你好，OpenVela" as the exact partial transcript "你好 open".
  // Accept only that complete folded utterance. Bare "open", "你好", and
  // strings with extra suffixes remain rejected, keeping this accommodation
  // confined to the wake-word grammar.
  if (actual == "你好open") return true;
...
bool isWakeWord(const std::string& text, const std::string& configured) {
  ...
  const std::string actual = foldWakePhrase(text);
  return actual == "你好open" || matchesWakePhrase(actual, wanted);
}
