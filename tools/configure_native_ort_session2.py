...
R=Path(__file__).resolve().parents[1];P=R/'private/native-ort-session-config2';P.mkdir(exist_ok=True)
O=R/'evidence/native-ort-session-config21-20260911';O.mkdir(exist_ok=True)
...
from pathlib import Path
S=Path('/dev/shm/velavision-ort-session-20260911');O=S/'config-attempt2';O.mkdir()
...
def patch(name,file,tree):
 ...
 p=subprocess.run(['patch','--dry-run','--binary','--ignore-whitespace','-p1'],input=data,cwd=tree,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
 (O/(name+'-check.log')).write_bytes(p.stdout)
 if p.returncode:raise RuntimeError(p.stdout.decode())
 p=subprocess.run(['patch','--binary','--ignore-whitespace','-p1'],input=data,cwd=tree,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
 (O/(name+'.log')).write_bytes(p.stdout);assert p.returncode==0
 records.append(dict(name=name,patch_sha256=hashlib.sha256(data).hexdigest(),exit_code=0))
try:
...
call(['scp.exe',*options,host+':'+dest+'/config-attempt2/results.tar.gz',str(P/'results.tar.gz')])
