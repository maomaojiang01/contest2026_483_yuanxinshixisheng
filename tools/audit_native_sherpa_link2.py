...
P = R / 'private/native-sherpa-link2'
O = R / 'evidence/native-sherpa-link2-20260911'
...
O=Path('/home/swl/openvela/work/native-sherpa-config2-20260911/link2')
...
ASR=Path('/home/swl/openvela/work/native-sherpa-config2-20260911')
assert json.loads((ASR/'compile-attempt3/result.json').read_text())['exit_code']==0
...
cmd=[ld,'-r','--undefined=SherpaOnnxCreateOnlineRecognizer','--strip-debug','-o',str(O/'session.o'),str(probe),'--start-group',*[str(x) for x in archives],'--end-group']
p=subprocess.run(cmd,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,timeout=180)
(O/'link.log').write_bytes(p.stdout)
result=dict(exit_code=p.returncode,command=cmd,inputs=inputs,firmware_linked=False,model_run=False,asr_ready=False,ort_kernel_set='Add-only; symbol inventory only')
if p.returncode==0:
...
subprocess.run(['scp.exe',*options,str(P/'audit.py'),host+':'+dest+'/audit-sherpa-capi2.py'],check=True)
subprocess.run(['ssh.exe',*options,host,'python3 '+dest+'/audit-sherpa-capi2.py'],check=True)
subprocess.run(['scp.exe',*options,host+':/home/swl/openvela/work/native-sherpa-config2-20260911/link2/results.tar.gz',str(P/'results.tar.gz')],check=True)
