...
P=R/'private/native-ort-reduced-locale';P.mkdir(exist_ok=True)
O=R/'evidence/native-ort-reduced-locale-20260911';O.mkdir(exist_ok=True)
C=R/'work-in-progress/parallel-model-reader-medium/native-iconv-reduced-v1'
remote=r'''
import hashlib,json,shlex,subprocess,tarfile
from pathlib import Path
S=Path('/dev/shm/velavision-ort-session-20260911');O=S/'locale-config';O.mkdir()
...
for src,name in [(P/'configure.py','locale-config.py'),(P/'reduced-locale.patch','reduced-locale.patch'),(C/'k7_verify_no_string_normalizer.py','k7_verify_no_string_normalizer.py')]:
...
subprocess.run(['scp.exe',*options,host+':'+dest+'/locale-config/results.tar.gz',str(P/'results.tar.gz')],check=True)
