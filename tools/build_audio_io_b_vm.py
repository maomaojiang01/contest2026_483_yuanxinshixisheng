...
old=json.loads((R/'private/audio-io-b-original-hashes.json').read_text(encoding='utf-8-sig'))
manifest={p:dict(new=sha(R/p),old=old.get(p)) for p in paths}
(STAGE/'staging.json').write_text(json.dumps(manifest))
remote=r'''
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/home/swl/openvela/work/audio-io-b-stage-20260910')
P=Path('/home/swl/openvela/work/velavision-project')
SDK=P.parents[1];O=SDK/'work/velavision-audio-io-b-20260910';O.mkdir(exist_ok=True)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
...
build=run('build',['bash',str(P/'tools/build_audio_io_b.sh'),str(SDK)])
B=Path('/dev/shm/velavision_audio_io_20260910')
assert all(x in (B/'.config').read_text().splitlines() for x in ['CONFIG_EXAMPLES_K7AUDIOHW=y','CONFIG_EXAMPLES_K7AUDIO=y','CONFIG_USBHOST_MSC=y','CONFIG_USBHOST_MSC_READONLY=y','CONFIG_EXAMPLES_K7STORAGE=y','CONFIG_EXAMPLES_K7ARENA=y','CONFIG_EXAMPLES_K7GRAPH=y','CONFIG_EXAMPLES_K7EH=y','CONFIG_CXX_WCHAR=y','CONFIG_CXX_MINI_LOCALIZATION=y','CONFIG_FS_LARGEFILE=y','CONFIG_EXAMPLES_K7NEON=y','CONFIG_EXAMPLES_K7CXX=y','CONFIG_HAVE_CXX=y','CONFIG_LIBCXX=y','CONFIG_LIBCXXABI=y','CONFIG_CXX_EXCEPTION=y','CONFIG_CXX_RTTI=y','CONFIG_TLS_NELEM=8','CONFIG_TLS_TASK_NELEM=8','CONFIG_EXAMPLES_K7LOAD=y','CONFIG_SMP=y','CONFIG_SMP_NCPUS=8','CONFIG_NCPUS=8','CONFIG_RK3576_SMP_DIAG=y','CONFIG_EXAMPLES_K7SMP=y','CONFIG_BOARDCTL_RESET=y','CONFIG_SMP_DEFAULT_CPUSET=0x1','CONFIG_EXAMPLES_K7RADIO_IP=y','CONFIG_BCH_DEVICE_READONLY=y'])
assert 'CONFIG_FS_FAT=y' not in (B/'.config').read_text().splitlines()
report=dict(revision='audio-io-b-20260910',build_exit_code=0,hardware_tested=False,sync_audit=audit,
...
profile='board/kickpi_k7/configs/velavision_audio_io_local/defconfig'
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/audio-io-b-20260910/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/audio-io-b-20260910/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/audio-io-b-stage-20260910'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-audio-io-b-20260910/result.tar.gz',str(result)])
