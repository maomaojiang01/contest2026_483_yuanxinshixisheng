...
R=Path(__file__).resolve().parents[1];P=R/'private/native-ort-session-config5';P.mkdir(exist_ok=True)
O=R/'evidence/native-ort-session-config51-20260911';O.mkdir(exist_ok=True)
...
from pathlib import Path
S=Path('/dev/shm/velavision-ort-session-20260911');O=S/'config-attempt5';O.mkdir()
...
def run(name,cmd,cwd):
 p=subprocess.run(['bash','-c','source /home/swl/openvela/build/envsetup.sh >/dev/null && '+shlex.join(cmd)],cwd=cwd,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,timeout=240)
...
call(['scp.exe',*options,host+':'+dest+'/config-attempt5/results.tar.gz',str(P/'results.tar.gz')])
