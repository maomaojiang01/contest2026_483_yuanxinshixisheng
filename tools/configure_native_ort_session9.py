...
R=Path(__file__).resolve().parents[1];P=R/'private/native-ort-session-config9';P.mkdir(exist_ok=True)
O=R/'evidence/native-ort-session-config91-20260911';O.mkdir(exist_ok=True)
...
from pathlib import Path
S=Path('/dev/shm/velavision-ort-session-20260911');O=S/'config-attempt9';O.mkdir()
CMAKE='/dev/shm/cmake-3.28.3-linux-x86_64/bin/cmake'
records=[]
assert hashlib.sha256((S/'ort/orttraining/orttraining/training_ops/cpu/cpu_training_kernels.cc').read_bytes()).hexdigest()=='d300d50a8d32f3b5a54170e9417fae5cffcd6839475e3d21f1873fa9fd72ab0f'
def run(name,cmd,cwd):
 p=subprocess.run(['bash','-c','cd /home/swl/openvela && source build/envsetup.sh >/dev/null && cd '+shlex.quote(str(cwd))+' && '+shlex.join(cmd)],cwd=cwd,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,timeout=240)
...
try:
 ...
 ar=str(Path(args[0]).with_name(Path(args[0]).name.replace('gcc','ar')))
 ...
 # Native CPU-only static runtime does not build the dynamic provider loader library.
 provider=S/'ort/cmake/onnxruntime_providers_cpu.cmake'
 before=provider.read_bytes()
 assert hashlib.sha256(before).hexdigest()=='17c838003562edbb50e0021cf6a5e8163aa306a046ec6845cdf00aa41f7f697a'
 text=before.decode()
 ...
 if good:
  cmd=[CMAKE,'-S',str(S/'ort/cmake'),'-B',str(S/'build'),'-G','Ninja','-C',str(S/'initial-cache.cmake'),
   '-DCMAKE_TOOLCHAIN_FILE='+str(S/'session-toolchain.cmake'),'-DCMAKE_BUILD_TYPE=MinSizeRel','-DIconv_INCLUDE_DIR=/home/swl/openvela/nuttx/include','-DIconv_LIBRARY='+str(O/'libk7_iconv.a'),
...
with tarfile.open(O/'results.tar.gz','w:gz') as tar:
 for file in O.iterdir():
  if file.suffix in ('.log','.json','.c','.h'):tar.add(file,arcname=file.name)
...
with tarfile.open(P/'stage.tar.gz','w:gz') as tar:
 ...
 tar.add(R/'work-in-progress/native-voice-sources/onnxruntime-8f5c79cb63f09ef1302e85081093a3fe4da1bc7d/orttraining/orttraining/training_ops/cpu/cpu_training_kernels.cc',arcname='ort/orttraining/orttraining/training_ops/cpu/cpu_training_kernels.cc')
...
call(['scp.exe',*options,host+':'+dest+'/config-attempt9/results.tar.gz',str(P/'results.tar.gz')])
