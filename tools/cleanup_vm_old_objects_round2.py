...
E=R/'evidence/vm-space-cleanup-20260911-round2';E.mkdir(exist_ok=True)
remote=r'''
import hashlib,json,os,stat,sys,time
from pathlib import Path
B=Path('/home/swl/openvela/cmake_out')
O=Path('/home/swl/openvela/work/space-cleanup-20260911-round2');O.mkdir(exist_ok=True)
...
if sys.argv[1]=='plan':
 ...
 for d in sorted(B.iterdir()):
  if d.is_symlink() or not d.is_dir() or d.name=='velavision_voice_tls_20260911' or not d.name.startswith(('rk3576_','velavision_')):continue
...
for name in (['plan.json'] if a.mode=='plan' else ['applied.json','deleted.jsonl']):
 subprocess.run(['scp.exe',*options,'swl@192.168.152.131:/home/swl/openvela/work/space-cleanup-20260911-round2/'+name,str(E/name)],check=True)
