...
old=json.loads((R/'private/voice-shared-original-hashes.json').read_text(encoding='utf-8-sig'))
manifest={p:dict(new=sha(R/p),old=old.get(p)) for p in paths}
# Exact prior failed staging permits advancing only known mirror files.
prior=R/'evidence/voice-shared-20260910/attempt2-staging.json'
assert sha(prior)=='383d4f7d3a11f3bb61b6a237a56a2da30412a1c0b5959f2be7ae6a2e546a1e0b'
for p,v in json.loads(prior.read_text()).items():
 if p in manifest:manifest[p]['old']=v['new']
(STAGE/'staging.json').write_text(json.dumps(manifest))
remote=r'''
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/home/swl/openvela/work/voice-shared-stage-20260910')
P=Path('/home/swl/openvela/work/velavision-project')
SDK=P.parents[1];O=SDK/'work/velavision-voice-shared-20260910';O.mkdir(exist_ok=True)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/voice-shared-20260910/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/voice-shared-20260910/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/voice-shared-stage-20260910'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-voice-shared-20260910/result.tar.gz',str(result)])
