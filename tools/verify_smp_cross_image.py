...
def verify(build):
    build=Path(build).resolve();assert build==ROOT/'artifacts/smp-cross-20260910'
    report=json.loads((ROOT/'evidence/build/smp-cross-20260910/verification.json').read_text())
    ...
    assert 'CONFIG_SMP_NCPUS=5' in cfg
...
if __name__=='__main__':print(verify(ROOT/'artifacts/smp-cross-20260910'))
