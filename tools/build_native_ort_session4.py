...
R=Path(__file__).resolve().parents[1];P=R/'private/native-ort-session-build4';P.mkdir(exist_ok=True)
O=R/'evidence/native-ort-session-build41-20260911';O.mkdir(exist_ok=True)
remote=r'''
import hashlib,json,shlex,subprocess,tarfile,time
from pathlib import Path
S=Path('/dev/shm/velavision-ort-session-20260911');O=S/'compile-attempt4';O.mkdir()
...
call(['scp.exe',*options,host+':'+dest+'/compile-attempt4/results.tar.gz',str(P/'results.tar.gz')])
