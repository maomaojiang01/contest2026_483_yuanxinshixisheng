...
old=json.loads((R/'private/smp-diag-original-hashes.json').read_text(encoding='utf-8-sig'))
manifest={p:dict(new=sha(R/p),old=old.get(p)) for p in paths}
(STAGE/'staging.json').write_text(json.dumps(manifest))
remote=r'''
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/home/swl/openvela/work/smp-diag-stage-20260910')
P=Path('/home/swl/openvela/work/velavision-project')
SDK=P.parents[1];O=SDK/'work/velavision-smp-diag-20260910';O.mkdir(exist_ok=True)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
...
build=run('build',['bash',str(P/'tools/build_smp_diag.sh'),str(SDK)])
B=SDK/'cmake_out/velavision_smp_diag_20260910'
assert all(x in (B/'.config').read_text().splitlines() for x in ['CONFIG_SMP=y','CONFIG_SMP_NCPUS=2','CONFIG_RK3576_SMP_DIAG=y','CONFIG_EXAMPLES_K7SMP=y'])
report=dict(revision='smp-diag-20260910',build_exit_code=0,hardware_tested=False,sync_audit=audit,
...
profile='board/kickpi_k7/configs/velavision_smp_diag_local/defconfig'
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/smp-diag-20260910/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/smp-diag-20260910/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/smp-diag-stage-20260910'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-smp-diag-20260910/result.tar.gz',str(result)])
