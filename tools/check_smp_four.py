...
E=R/'evidence/smp-four-20260910'
...
passed=returned and any(x=='SMP result=PASS shared_counter=40000 expected=40000' for x in lines)
for cpu in range(4):
...
result=dict(passed=passed,prompt_returned=returned,lines=lines,storage_writes=False,
            scope='four A53 pinned workers, shared atomic counter and timed sleeps; not eight-core or peripheral SMP acceptance')
(E/'diagnostic.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result))
if not passed:raise RuntimeError('Four-core diagnostic not passed; preserve evidence before recovery')
