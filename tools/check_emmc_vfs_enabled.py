...
R=Path(__file__).resolve().parents[1];E=R/'evidence/emmc-vfs-20260910'
...
with s:
    for cmd in ('dd if=/dev/emmc0ro of=/dev/null bs=512 count=128','echo $?', 'dd if=/dev/emmc0ro of=/dev/null bs=1 skip=510 count=4', 'echo $?', 'k7emmc gpt'):
        ...
        for line in buf.decode('ascii',errors='replace').splitlines():
            ...
            if line == '0' or line.startswith(('dd:','nsh:','EMMC ','ERROR:')) or re.match(r'^\d+.*(?:records|bytes)',line):lines.append(line)
...
passed=len(records)==5 and all(x['prompt_returned'] for x in records) and records[1]['lines']==['0'] and records[3]['lines']==['0'] and not any('nsh:' in line or 'ERROR:' in line for x in records[:4] for line in x['lines'])
dest.write_text(json.dumps(dict(records=records,vfs_commands_passed=passed,requested_read_bytes=65536,unaligned_read_bytes=4,destination='/dev/null',storage_writes=False),indent=2)+'\n',encoding='utf-8')
print(json.dumps(records))

if not passed:raise RuntimeError('VFS read commands not verified')
