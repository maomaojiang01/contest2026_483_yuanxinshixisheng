#!/usr/bin/env bash
set -euo pipefail
project_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
sdk_dir="${1:?Usage: bash tools/build.sh /absolute/path/to/openvela}"
python3 "$project_dir/tools/sync_sdk.py" --sdk "$sdk_dir" --apply
cd "$sdk_dir"
# The SDK environment script probes optional unset variables.
set +eu
source build/envsetup.sh >/dev/null
setup_status=$?
set -eu
if [ "$setup_status" -ne 0 ]; then
  echo "SDK environment setup failed: $setup_status" >&2
  exit "$setup_status"
fi
cmake -S nuttx -B cmake_out/velavision_integrated_20260909 -G Ninja \
  -DBOARD_CONFIG=kickpi_k7:velavision_integrated_local
cmake --build cmake_out/velavision_integrated_20260909 -j6
