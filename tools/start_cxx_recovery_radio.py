...
R=Path(__file__).resolve().parents[1];E=R/'evidence/cxx-tls-20260910'
...
# CPU topology baseline unchanged; this revision tests the bounded compute workload.
assert json.loads((E/'recovery-affinity-boot.json').read_text())['passed']
...
with s:
 ...
 for name,cmd,timeout,expected in stages:
  ...
  (E/('recovery-'+name+'.bin')).write_bytes(buf)
