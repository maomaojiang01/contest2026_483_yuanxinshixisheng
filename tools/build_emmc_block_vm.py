...
build=run('build',['bash',str(P/'tools/build_emmc_block.sh'),str(SDK)])
B=SDK/'cmake_out/velavision_emmc_block_20260910'
assert 'CONFIG_EXAMPLES_K7EMMC=y' in (B/'.config').read_text()
report=dict(revision='emmc-block-20260910',build_exit_code=0,hardware_tested=False,sync_audit=audit,
...
profile='board/kickpi_k7/configs/velavision_emmc_block_local/defconfig'
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/emmc-block-20260910/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/emmc-block-20260910/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/emmc-block-stage-20260910'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-emmc-block-20260910/result.tar.gz',str(result)])
