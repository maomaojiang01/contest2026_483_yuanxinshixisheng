...
assert state['done']==0 and not state['busy'] and state['held'],state
assert b'integration_panel.py' in Path('/proc/37503/cmdline').read_bytes()
os.kill(37503,signal.SIGTERM);time.sleep(.5)
...
(ROOT/'evidence/video-continuous-20260917/enter-download.log').write_bytes(out)
