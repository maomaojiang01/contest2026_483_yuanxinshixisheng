...
R=Path(__file__).resolve().parents[1];P=R/'private/native-ort-session-build2';P.mkdir(exist_ok=True)
O=R/'evidence/native-ort-session-build21-20260911';O.mkdir(exist_ok=True)
remote=r'''
import hashlib,json,shlex,subprocess,tarfile,time
from pathlib import Path
S=Path('/dev/shm/velavision-ort-session-20260911');O=S/'compile-attempt2';O.mkdir()
targets=['onnxruntime_session','onnxruntime_mlas','onnxruntime_framework','onnxruntime_optimizer',
 'onnxruntime_providers','onnxruntime_graph','onnxruntime_util','onnxruntime_common','onnxruntime_flatbuffers']
cmd=['/dev/shm/cmake-3.28.3-linux-x86_64/bin/cmake','--build',str(S/'build'),'--target',*targets,'-j2']
start=time.monotonic();code=None;timeout=False
try:
 with (O/'build.log').open('wb') as stream:
  p=subprocess.run(['bash','-c','cd /home/swl/openvela && source build/envsetup.sh >/dev/null && '+shlex.join(cmd)],stdout=stream,stderr=subprocess.STDOUT,timeout=600)
  code=p.returncode
...
call(['scp.exe',*options,host+':'+dest+'/compile-attempt2/results.tar.gz',str(P/'results.tar.gz')])
