...
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/home/swl/openvela/work/neon-file64-stage-20260910')
P=Path('/home/swl/openvela/work/velavision-project')
SDK=P.parents[1];O=SDK/'work/velavision-neon-file64-20260910';O.mkdir(exist_ok=True)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
...
profile='board/kickpi_k7/configs/velavision_neon_file64_local/defconfig'
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/neon-file64-20260910/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/neon-file64-20260910/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/neon-file64-stage-20260910'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-neon-file64-20260910/result.tar.gz',str(result)])
