"""Verify the exact phone-rpa build and ELF load bounds before RAM boot."""
import hashlib,json,struct
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
def verify(build):
    build=Path(build).resolve();assert build==ROOT/'artifacts/phone-rpa-20260909'
    report=json.loads((ROOT/'evidence/build/phone-rpa-20260909/verification.json').read_text())
...
if __name__=='__main__':print(verify(ROOT/'artifacts/phone-rpa-20260909'))
