...
FILES=['app/k7host/k7_pipeline.c','app/k7host/k7_pipeline.h','app/k7host/k7_photo.c','app/k7host/k7_photo.h','app/k7agent/cloud/src/board_speech_bridge.c']
...
sdk=pathlib.Path('/home/swl/openvela');root=sdk/'work/velavision-project';ev=root/'evidence/report-follow-20260917'
...
with (ev/'build.log').open('xb') as log:
 shell='source build/envsetup.sh >/dev/null && prebuilts/tools/cmake/bin/cmake -S nuttx -B cmake_out/velavision_report_follow_20260917 -G Ninja -DBOARD_CONFIG=kickpi_k7:velavision_photo_tcp && prebuilts/tools/cmake/bin/cmake --build cmake_out/velavision_report_follow_20260917 -j4; result=$?; echo $result > work/velavision-project/evidence/report-follow-20260917/build.exit; exit $result'
...
sdk=pathlib.Path('/home/swl/openvela');ev=sdk/'work/velavision-project/evidence/report-follow-20260917'
assert (ev/'build.exit').read_text().strip()=='0'
raw=(sdk/'cmake_out/velavision_report_follow_20260917/nuttx.bin').read_bytes()
