...
p.add_argument('--resume-preserved-snapshot', action='store_true',
               help='Keep 0x51000000 snapshot after an interrupted load; CRC-check every reused block')
p.add_argument('--port', default='COM8')
a = p.parse_args()
build = a.build.resolve()
result = verify(build)
payload = (build / 'nuttx.bin').read_bytes()
# Optional in-place delta is enabled only after an exact old block CRC match.
old_build = build.parent / 'voice-ort-add-20260911'
old_report = __import__('json').loads((build.parents[1] / 'evidence/build/voice-ort-add-20260911/verification.json').read_text())
old_image = (old_build / 'nuttx.bin').read_bytes()
assert hashlib.sha256(old_image).hexdigest() == old_report['artifacts']['nuttx.bin']['sha256']
...
previous_build = build.parent / 'voice-owner-20260910'
import json
previous_report = json.loads((Path(__file__).resolve().parents[1] / 'evidence/build/voice-owner-20260910/verification.json').read_text())
...
previous_chunks = {previous[p:p+4096]: p for p in range(0,len(previous),4096)}
reuse = {p: previous_chunks.get(payload[p:p+4096], -1) for p in range(0,len(payload),4096)}
base = 0x40400000
assert result['entry'] == hex(base)
assert 0 < len(payload) < result['memory_bytes'] and len(payload) % 8 == 0
print('IMAGE', len(payload), hashlib.sha256(payload).hexdigest(), flush=True)
assert build.name == 'voice-ort-trace-20260911'
...
with s, a.log.open('ab') as log:
    ...
    if a.resume_preserved_snapshot:
        # Abort an unfinished console line, not the preserved RAM contents.
        s.write(b'\x03\r')
        s.flush()
        time.sleep(.2)
        log.write(s.read(8192))
    ...
    if not a.resume_preserved_snapshot:
        command(f'cp.b 40400000 {snapshot_base:x} {len(previous):x}')
    else:
        print('RESUME: preserved snapshot; destination and source blocks require CRC', flush=True)
            ...
            if not crc_ok(base + offset, chunk):
                ...
                if source_offset >= 0 and crc_ok(snapshot_base + source_offset, chunk):
                    ...
                    if crc_ok(base + offset, chunk):
                        ...
                        continue
                old_chunk = old_image[offset:offset+len(chunk)] if region_name == 'openvela' else b''
                delta = len(old_chunk) == len(chunk) and crc_ok(base + offset, old_chunk)
                for attempt in range(3):
                    commands = []
                    pos = 0
                    while pos < len(chunk):
                        if delta and attempt == 0 and old_chunk[pos:pos+8] == chunk[pos:pos+8]:
                            pos += 8
                            continue
