...
R=Path(__file__).resolve().parents[1];P=R/'private/native-ort-session-config7';P.mkdir(exist_ok=True)
O=R/'evidence/native-ort-session-config71-20260911';O.mkdir(exist_ok=True)
...
from pathlib import Path
S=Path('/dev/shm/velavision-ort-session-20260911');O=S/'config-attempt7';O.mkdir()
...
call(['scp.exe',*options,host+':'+dest+'/config-attempt7/results.tar.gz',str(P/'results.tar.gz')])
