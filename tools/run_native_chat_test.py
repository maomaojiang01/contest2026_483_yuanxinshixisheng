...
stamp=datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ');out=ROOT/'evidence/native-chat-20260915'/('human-'+stamp+'.log')
with out.open('wb') as log:
 ...
 while True:
  ...
  if not line:break
  log.write(line);log.flush();print(line.decode(errors='replace'),end='',flush=True)
 result=child.wait()
