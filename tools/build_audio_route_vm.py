...
old=json.loads((R/'private/audio-route-original-hashes.json').read_text(encoding='utf-8-sig'))
manifest={p:dict(new=sha(R/p),old=old.get(p)) for p in paths}
(STAGE/'staging.json').write_text(json.dumps(manifest))
remote=r'''
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/home/swl/openvela/work/audio-route-stage-20260910')
P=Path('/home/swl/openvela/work/velavision-project')
SDK=P.parents[1];O=SDK/'work/velavision-audio-route-20260910';O.mkdir(exist_ok=True)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
...
build=run('build',['bash',str(P/'tools/build_audio_route.sh'),str(SDK)])
...
report=dict(revision='audio-route-20260910',build_exit_code=0,hardware_tested=False,sync_audit=audit,
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/audio-route-20260910/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/audio-route-20260910/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/audio-route-stage-20260910'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-audio-route-20260910/result.tar.gz',str(result)])
