...
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/home/swl/openvela/work/cxx-probe-stage-20260910')
P=Path('/home/swl/openvela/work/velavision-project')
SDK=P.parents[1];O=SDK/'work/velavision-cxx-probe-20260910';O.mkdir(exist_ok=True)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
...
build=run('build',['bash',str(P/'tools/build_cxx_probe.sh'),str(SDK)])
B=SDK/'cmake_out/velavision_cxx_probe_20260910'
assert all(x in (B/'.config').read_text().splitlines() for x in ['CONFIG_EXAMPLES_K7CXX=y','CONFIG_HAVE_CXX=y','CONFIG_LIBCXX=y','CONFIG_LIBCXXABI=y','CONFIG_CXX_EXCEPTION=y','CONFIG_CXX_RTTI=y','CONFIG_TLS_NELEM=8','CONFIG_EXAMPLES_K7LOAD=y','CONFIG_SMP=y','CONFIG_SMP_NCPUS=8','CONFIG_NCPUS=8','CONFIG_RK3576_SMP_DIAG=y','CONFIG_EXAMPLES_K7SMP=y','CONFIG_BOARDCTL_RESET=y','CONFIG_SMP_DEFAULT_CPUSET=0x1','CONFIG_EXAMPLES_K7RADIO_IP=y','CONFIG_BCH_DEVICE_READONLY=y'])
report=dict(revision='cxx-probe-20260910',build_exit_code=0,hardware_tested=False,sync_audit=audit,
...
profile='board/kickpi_k7/configs/velavision_cxx_probe_local/defconfig'
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/cxx-probe-20260910/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/cxx-probe-20260910/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/cxx-probe-stage-20260910'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-cxx-probe-20260910/result.tar.gz',str(result)])
