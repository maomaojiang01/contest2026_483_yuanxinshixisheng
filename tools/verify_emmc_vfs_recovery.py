...
def verify(build):
    build=Path(build).resolve();assert build==ROOT/'artifacts/emmc-vfs-20260910'
    report=json.loads((ROOT/'evidence/build/emmc-vfs-20260910/verification.json').read_text())
    assert report['build_exit_code']==0
    # Recovery intentionally restores the previously tested artifact, not current sources.
    tested=json.loads((ROOT/'evidence/emmc-vfs-20260910/device-state.json').read_text())
    assert tested['running_revision']=='emmc-vfs-20260910'
    assert tested['firmware_sha256']==report['artifacts']['nuttx.bin']['sha256']
    assert tested['liveness_verified'] and tested['emmc']['vfs_read_passed']
...
if __name__=='__main__':print(verify(ROOT/'artifacts/emmc-vfs-20260910'))
