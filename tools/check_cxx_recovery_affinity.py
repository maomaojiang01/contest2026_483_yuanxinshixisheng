...
E = R / 'evidence/cxx-tls-20260910'
p = argparse.ArgumentParser()
p.add_argument('--radio', action='store_true')
a = p.parse_args()
name = 'recovery-affinity-radio' if a.radio else 'recovery-affinity-boot'
...
import serial
s = serial.Serial(port=None, baudrate=console_baud(R, 'smp-load-20260910'), timeout=.02, write_timeout=2)
s.rts = False
s.dtr = False
s.port = 'COM8'
raw = bytearray()
