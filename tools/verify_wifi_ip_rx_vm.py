...
OUT=SDK/'work/velavision-wifi-ip-rx-20260909';OUT.mkdir(exist_ok=True)
p=subprocess.run(['python3',str(PROJECT/'tools/test_supplicant_vm.py')],capture_output=True,text=True)
report=dict(revision='wifi-ip-rx-20260909',hardware_tested=False,handshake_tests=dict(exit_code=p.returncode,output=p.stdout+p.stderr))
