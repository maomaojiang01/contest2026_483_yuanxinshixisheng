...
E = R/'evidence/neon-file64-20260910'
assert not (E/'runtime.json').exists(), 'Do not overwrite a completed attempt'
assert 'PASS: REAL K7 NSH' in (E/'ramload-resume-progress.txt').read_text(encoding='utf-8-sig')
...
report = dict(passed=bool(prompt and match and match[1]==b'PASS' and int(match[3])==64),
