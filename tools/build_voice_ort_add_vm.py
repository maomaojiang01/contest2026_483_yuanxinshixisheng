...
R=Path(__file__).resolve().parents[1];REV='voice-ort-add-20260911'
P=R/'private'/REV;P.mkdir(exist_ok=True)
O=R/'evidence/build'/REV;O.mkdir(parents=True,exist_ok=False)
prior=json.loads((R/'evidence/build/voice-owner-20260910/verification.json').read_text())
...
runtime=json.loads((R/'evidence/native-session-link2-20260911/result.json').read_text())
...
S=SDK/'work/voice-ort-add-stage-20260911';O=SDK/'work/voice-ort-add-evidence-20260911';O.mkdir()
B=SDK/'cmake_out/velavision_voice_ort_add_20260911';assert not B.exists()
...
rt=json.loads((T/'link-attempt2/result.json').read_text());assert rt['missing_count']==0
obj=T/'link-attempt2/session.o';assert sha(obj)==rt['relocatable_sha256']
dest=SDK/'work/native-ort-add-runtime-20260911';dest.mkdir(exist_ok=True)
...
result=dict(revision='voice-ort-add-20260911',build_exit_code=0,hardware_tested=False,
...
with tarfile.open(O/'results.tar.gz','w:gz') as t:
 for x in O.iterdir():
  if x.suffix in ('.json','.log'):t.add(x,arcname='evidence/build/voice-ort-add-20260911/'+x.name)
 for n in ('nuttx','nuttx.bin','.config'):t.add(B/n,arcname='artifacts/voice-ort-add-20260911/'+n)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/voice-ort-add-stage-20260911'
...
subprocess.run(['scp.exe',*options,host+':/home/swl/openvela/work/voice-ort-add-evidence-20260911/results.tar.gz',str(P/'results.tar.gz')],check=True)
