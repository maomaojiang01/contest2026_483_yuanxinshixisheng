...
FILES=['app/k7agent/cloud/src/board_speech_bridge.c','board/kickpi_k7/configs/velavision_photo_tcp/defconfig']
...
r=pathlib.Path("/home/swl/openvela")
def dest(p):return r/('apps/examples/'+p[4:] if p.startswith('app/') else 'nuttx/boards/arm64/rk3576/'+p[6:])
print(json.dumps({p:base64.b64encode(dest(p).read_bytes()).decode() if dest(p).exists() else None for p in %r}))'''%FILES))
    hashes={};diff=[]
...
sdk=pathlib.Path('/home/swl/openvela');root=sdk/'work/velavision-project';ev=root/'evidence/photo-tcp-20260916'
assert not (ev/'build.log').exists(),'build already exists'
for rel,digest in %r.items():
 p=sdk/('apps/examples/'+rel[4:] if rel.startswith('app/') else 'nuttx/boards/arm64/rk3576/'+rel[6:])
...
with (ev/'build.log').open('xb') as log:
 shell='source build/envsetup.sh >/dev/null && prebuilts/tools/cmake/bin/cmake -S nuttx -B cmake_out/velavision_photo_tcp_20260916 -G Ninja -DBOARD_CONFIG=kickpi_k7:velavision_photo_tcp && prebuilts/tools/cmake/bin/cmake --build cmake_out/velavision_photo_tcp_20260916 -j4; result=$?; echo $result > work/velavision-project/evidence/photo-tcp-20260916/build.exit; exit $result'
...
sdk=pathlib.Path('/home/swl/openvela');ev=sdk/'work/velavision-project/evidence/photo-tcp-20260916'
assert (ev/'build.exit').read_text().strip()=='0'
raw=(sdk/'cmake_out/velavision_photo_tcp_20260916/nuttx.bin').read_bytes()
assert all(s in raw for s in [b'VOICE SESSION ready mode=0',b'K7CLOUD prompt_started busy=1',b'K7CLOUD tts_state'])
print(json.dumps(dict(image=base64.b64encode(raw).decode(),sha256=hashlib.sha256(raw).hexdigest(),size=len(raw),log=(ev/'build.log').read_text(errors='replace'),sources={p:hashlib.sha256((sdk/('apps/examples/'+p[4:] if p.startswith('app/') else 'nuttx/boards/arm64/rk3576/'+p[6:])).read_bytes()).hexdigest() for p in %r})))
'''%(FILES+MOTION_FILES)))
