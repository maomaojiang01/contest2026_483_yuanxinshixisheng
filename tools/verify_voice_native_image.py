...
def verify(build):
    ...
    for line in ['CONFIG_EXAMPLES_K7VOICE=y','CONFIG_EXAMPLES_K7RADIO_SHARED=y','CONFIG_EXAMPLES_K7SOUND=y','CONFIG_EXAMPLES_K7AUDIO=y','CONFIG_USBHOST_MSC=y','CONFIG_USBHOST_MSC_READONLY=y','CONFIG_EXAMPLES_K7STORAGE=y','CONFIG_EXAMPLES_K7ARENA=y','CONFIG_EXAMPLES_K7GRAPH=y','CONFIG_CXX_WCHAR=y','CONFIG_CXX_MINI_LOCALIZATION=y','CONFIG_FS_LARGEFILE=y','CONFIG_EXAMPLES_K7NEON=y','CONFIG_EXAMPLES_K7CXX=y','CONFIG_LIBCXX=y','CONFIG_LIBCXXABI=y',
    ...
    assert 'CONFIG_FS_FAT=y' not in cfg
    audit = json.loads((ROOT/'evidence/build'/REV/'unwind-audit.json').read_text())
    assert audit['passed'] and audit['sha256'] == report['artifacts']['nuttx']['sha256']
    layout=json.loads((ROOT/'evidence/build'/REV/'layout-audit.json').read_text())
    ...
    for i in range(count):
        ...
        assert entry <= physical <= physical+memsz <= 0x40c00000
