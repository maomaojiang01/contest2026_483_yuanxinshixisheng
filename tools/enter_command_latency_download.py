...
assert state['held'] and state['done']==7 and 'mock' in state['upload'],state
assert b'integration_panel.py' in Path('/proc/64308/cmdline').read_bytes()
os.kill(64308,signal.SIGTERM);time.sleep(.5)
...
with s:
 ...
 s.write(b'k7cloud device-stop'+bytes([13]));s.flush();readfor(4)
 s.write(b'k7cloud tts-stop'+bytes([13]));s.flush();readfor(1)
 s.write(b'k7cloud tts-state'+bytes([13]));s.flush();status=readfor(1)
 assert b'busy=0' in status,'Audio still busy; no reboot sent'
...
(ROOT/'evidence/command-latency-20260917/enter-download.log').write_bytes(out)
