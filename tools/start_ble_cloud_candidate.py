...
out=p/'evidence/ble-cloud-start-20260914';out.mkdir(exist_ok=True)
fw=pathlib.Path('/home/swl/openvela/cmake_out/velavision_capture_stream_20260914/nuttx.bin').read_bytes()
assert len(fw)==2782928 and hashlib.sha256(fw).hexdigest()=='5c4d2c0d07870ad2fc787ae3731bf12ee6defe912189cb4123ae83a7ae119e45'
...
with r.open_port() as s:
 ...
 def nsh(cmd,timeout):
  s.write(cmd.encode()+b'\r');s.flush();b=bytearray();end=time.monotonic()+timeout
  while time.monotonic()<end:
   b.extend(s.read(8192))
   if b'nsh>' in b: break
  else: raise RuntimeError('NSH timeout: '+cmd)
  (out/(cmd.split()[0]+'-'+str(time.time_ns())+'.log')).write_bytes(b)
  print(b.decode(errors='replace'),flush=True);return b
...
if __name__ == '__main__':
    out=ROOT/'evidence/ble-cloud-start-20260914'
