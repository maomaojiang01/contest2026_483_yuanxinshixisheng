...
import serial
p=argparse.ArgumentParser();p.add_argument('--probe',action='store_true');p.add_argument('--blockcheck',action='store_true');p.add_argument('--out',type=Path,required=True)
a=p.parse_args()
assert not a.out.exists(), 'Use a new evidence filename'
E=R/'evidence/emmc-block-20260910'
...
buf=bytearray();returned=False;cmd='k7emmc '+('blockcheck' if a.blockcheck else 'register' if a.probe else 'status')
try:
    with s:
        s.write(cmd.encode('ascii')+b'\r');s.flush();end=time.monotonic()+16
