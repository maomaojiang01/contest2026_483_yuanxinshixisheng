...
def verify(build):
    build=Path(build).resolve();assert build==ROOT/'artifacts/ble-connect-20260909'
    report=json.loads((ROOT/'evidence/build/ble-connect-20260909/verification.json').read_text())
    assert report['build_exit_code']==0 and report['protocol_tests']['exit_code']==0
...
if __name__=='__main__':print(verify(ROOT/'artifacts/ble-connect-20260909'))
