...
def verify(build):
    build=Path(build).resolve();assert build==ROOT/'artifacts/smp-load-20260910'
    report=json.loads((ROOT/'evidence/build/smp-load-20260910/verification.json').read_text())
    ...
    assert 'CONFIG_SMP_NCPUS=8' in cfg
    assert 'CONFIG_NCPUS=8' in cfg
    assert 'CONFIG_EXAMPLES_K7LOAD=y' in cfg
    assert 'CONFIG_SMP_DEFAULT_CPUSET=0x1' in cfg
...
if __name__=='__main__':print(verify(ROOT/'artifacts/smp-load-20260910'))
