...
OUT=SDK/'work/velavision-wifi-auth-20260909';OUT.mkdir(exist_ok=True)
p=subprocess.run(['python3',str(PROJECT/'tools/test_supplicant_vm.py')],capture_output=True,text=True)
report=dict(revision='wifi-auth-20260909',hardware_tested=False,handshake_tests=dict(exit_code=p.returncode,output=p.stdout+p.stderr))
print(p.stdout+p.stderr,flush=True)
assert p.returncode==0
with (OUT/'build.log').open('w') as log:
    p=subprocess.run(['bash',str(PROJECT/'tools/build_wifi_auth.sh'),str(SDK)],stdout=log,stderr=subprocess.STDOUT)
report['build_exit_code']=p.returncode
if not p.returncode:
    build=SDK/'cmake_out/velavision_wifi_auth_20260909'
