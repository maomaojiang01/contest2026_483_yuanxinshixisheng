...
FILES=['app/k7host/'+p for p in ['CMakeLists.txt','k7_pipeline.c','k7_photo.c','k7_photo.h','k7_photo_store.c','k7_photo_store.h']]
REV='native-photo-20260915'
...
sdk=pathlib.Path('/home/swl/openvela');root=sdk/'work/velavision-project';ev=root/'evidence/native-photo-20260915'
...
with (ev/'build.log').open('xb') as log:
 shell='source build/envsetup.sh >/dev/null && prebuilts/tools/cmake/bin/cmake -S nuttx -B cmake_out/velavision_native_photo_20260915 -G Ninja -DBOARD_CONFIG=kickpi_k7:velavision_cloud_speech_buffered && prebuilts/tools/cmake/bin/cmake --build cmake_out/velavision_native_photo_20260915 -j4; result=$?; echo $result > work/velavision-project/evidence/native-photo-20260915/build.exit; exit $result'
...
sdk=pathlib.Path('/home/swl/openvela');ev=sdk/'work/velavision-project/evidence/native-photo-20260915'
assert (ev/'build.exit').read_text().strip()=='0'
raw=(sdk/'cmake_out/velavision_native_photo_20260915/nuttx.bin').read_bytes()
