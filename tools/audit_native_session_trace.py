...
P = R / 'private/native-session-trace'
O = R / 'evidence/native-session-trace-20260911'
...
probe=Path('/dev/shm/velavision-native-add-trace-20260911/add_probe.o')
...
subprocess.run(['scp.exe',*options,str(P/'audit.py'),host+':'+dest+'/audit-session-trace.py'],check=True)
subprocess.run(['ssh.exe',*options,host,'python3 '+dest+'/audit-session-trace.py'],check=True)
subprocess.run(['scp.exe',*options,host+':'+dest+'/link-trace1/results.tar.gz',str(P/'results.tar.gz')],check=True)
