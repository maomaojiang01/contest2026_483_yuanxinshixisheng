...
paths += [p.relative_to(R).as_posix() for p in sorted((R/'app/k7radio').rglob('*')) if p.is_file()]
paths.append('evidence/sync/voice-shared-attempt2.json')
paths += [p.relative_to(R).as_posix() for p in sorted((R/'app/voicelink').rglob('*')) if p.is_file()]
paths.append('evidence/sync/voice-owner-attempt1.json')
paths=list(dict.fromkeys(p.replace('\\','/') for p in paths))
old=json.loads((R/'private/voice-owner-original-hashes.json').read_text(encoding='utf-8-sig'))
manifest={p:dict(new=sha(R/p),old=old.get(p)) for p in paths}
failed=json.loads((R/'evidence/sync/voice-owner-attempt1.json').read_text())
for rel in ('app/k7radio/radio_backend.inc','tools/sync_sdk.py'):
    manifest[rel]['old']=failed[rel]['new']
(STAGE/'staging.json').write_text(json.dumps(manifest))
remote=r'''
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/home/swl/openvela/work/voice-owner-stage-20260910')
P=Path('/home/swl/openvela/work/velavision-project')
SDK=P.parents[1];O=SDK/'work/velavision-voice-owner-20260910';O.mkdir(exist_ok=True)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
...
build=run('build',['bash',str(P/'tools/build_voice_owner.sh'),str(SDK)])
B=Path('/dev/shm/velavision_audio_sound_20260910')
assert all(x in (B/'.config').read_text().splitlines() for x in ['CONFIG_EXAMPLES_K7VOICE=y','CONFIG_EXAMPLES_K7RADIO_SHARED=y','CONFIG_EXAMPLES_K7SOUND=y','CONFIG_EXAMPLES_K7AUDIO=y','CONFIG_USBHOST_MSC=y','CONFIG_USBHOST_MSC_READONLY=y','CONFIG_EXAMPLES_K7STORAGE=y','CONFIG_EXAMPLES_K7ARENA=y','CONFIG_EXAMPLES_K7GRAPH=y','CONFIG_EXAMPLES_K7EH=y','CONFIG_CXX_WCHAR=y','CONFIG_CXX_MINI_LOCALIZATION=y','CONFIG_FS_LARGEFILE=y','CONFIG_EXAMPLES_K7NEON=y','CONFIG_EXAMPLES_K7CXX=y','CONFIG_HAVE_CXX=y','CONFIG_LIBCXX=y','CONFIG_LIBCXXABI=y','CONFIG_CXX_EXCEPTION=y','CONFIG_CXX_RTTI=y','CONFIG_TLS_NELEM=8','CONFIG_TLS_TASK_NELEM=8','CONFIG_EXAMPLES_K7LOAD=y','CONFIG_SMP=y','CONFIG_SMP_NCPUS=8','CONFIG_NCPUS=8','CONFIG_RK3576_SMP_DIAG=y','CONFIG_EXAMPLES_K7SMP=y','CONFIG_BOARDCTL_RESET=y','CONFIG_SMP_DEFAULT_CPUSET=0x1','CONFIG_EXAMPLES_K7RADIO_IP=y','CONFIG_BCH_DEVICE_READONLY=y'])
assert 'CONFIG_FS_FAT=y' not in (B/'.config').read_text().splitlines()
report=dict(revision='voice-owner-20260910',build_exit_code=0,hardware_tested=False,sync_audit=audit,
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/voice-owner-20260910/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/voice-owner-20260910/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/voice-owner-stage-20260910'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-voice-owner-20260910/result.tar.gz',str(result)])
