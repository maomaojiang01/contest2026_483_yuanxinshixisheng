...
R=Path(__file__).resolve().parents[1];P=R/'private/native-buffer-loader';P.mkdir(exist_ok=True)
O=R/'evidence/native-buffer-loader-compile-20260911';O.mkdir(exist_ok=True)
C=R/'work-in-progress/native-model-buffer-loader'
remote=r'''
import hashlib,json,shlex,subprocess,tarfile
from pathlib import Path
S=Path('/dev/shm/velavision-native-buffer-loader-20260911')
...
cmd=[args[0],*flags,'-Werror','-I/dev/shm/velavision-ort-session-20260911/ort/include/onnxruntime/core/session',
 '-I'+str(S/'vendor'),'-c',str(S/'buffer_compile.c'),'-o',str(S/'buffer_loader.o')]
...
(S/'result.json').write_text(json.dumps(dict(exit_code=p.returncode,command=cmd,
 inputs={n:hashlib.sha256((S/n).read_bytes()).hexdigest() for n in ('buffer_compile.c','model_buffer_loader.c','model_buffer_loader.h','model_reader.c','model_reader.h','sha256_vendor.c','sha256_namespace.h','vendor/sha256/sha256.h','vendor/sha256/sha256.c','vendor/rotate-bits/rotate-bits.h')},
...
with tarfile.open(P/'stage.tar.gz','w:gz') as tar:
 for name in ('buffer_compile.c','model_buffer_loader.c','model_buffer_loader.h','model_reader.c','model_reader.h','sha256_vendor.c','sha256_namespace.h','vendor/sha256/sha256.h','vendor/sha256/sha256.c','vendor/rotate-bits/rotate-bits.h'):tar.add(C/name,arcname=name)
...
host='swl@192.168.152.131';dest='/dev/shm/velavision-native-buffer-loader-20260911'
