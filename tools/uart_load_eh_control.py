...
from verify_eh_control_image import verify
...
previous_build = build.parent / 'graph-core-20260910'
import json
previous_report = json.loads((Path(__file__).resolve().parents[1] / 'evidence/build/graph-core-20260910/verification.json').read_text())
...
assert 0 < len(payload) < 0x600000 and len(payload) % 8 == 0
print('IMAGE', len(payload), hashlib.sha256(payload).hexdigest(), flush=True)
assert build.name == 'eh-control-20260910'
