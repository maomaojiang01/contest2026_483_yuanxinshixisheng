...
FILES=['app/k7agent/cloud/src/board_speech_bridge.c','app/k7sound/k7sound_main.c','app/k7sound/capture_stream.h']
...
sdk=pathlib.Path('/home/swl/openvela');root=sdk/'work/velavision-project';ev=root/'evidence/native-chat-cue-20260915'
...
with (ev/'build.log').open('xb') as log:
 shell='source build/envsetup.sh >/dev/null && prebuilts/tools/cmake/bin/cmake -S nuttx -B cmake_out/velavision_native_chat_cue_20260915 -G Ninja -DBOARD_CONFIG=kickpi_k7:velavision_cloud_speech_buffered && prebuilts/tools/cmake/bin/cmake --build cmake_out/velavision_native_chat_cue_20260915 -j4; result=$?; echo $result > work/velavision-project/evidence/native-chat-cue-20260915/build.exit; exit $result'
...
sdk=pathlib.Path('/home/swl/openvela');ev=sdk/'work/velavision-project/evidence/native-chat-cue-20260915'
assert (ev/'build.exit').read_text().strip()=='0'
raw=(sdk/'cmake_out/velavision_native_chat_cue_20260915/nuttx.bin').read_bytes()
