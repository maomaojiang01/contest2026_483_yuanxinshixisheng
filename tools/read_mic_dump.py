...
try:
 ...
 data=bytearray();end=time.monotonic()+25
...
def absorb(data):
 for pos,values in re.findall(rb'^PCM ([0-9a-f]{6})((?: [0-9a-f]{8}){8})\r?\r?$',data,re.M):
  ...
  for i,v in enumerate(values.split()):
   ...
   words[at+i]=value
for name in ['mic-replay-fixed.log','mic-dump-buffered.log']:
 absorb((folder/name).read_bytes())
for attempt in range(8):
 if len(words)==96000:break
 data=base64.b64decode(remote(code))
 (folder/('mic-dump-extra-%d.log'%attempt)).write_bytes(data)
 absorb(data);print('attempt',attempt,'valid_words',len(words),flush=True)
(folder/'mic-dump-coverage.json').write_text(json.dumps({'words':len(words),'expected':96000,'complete':len(words)==96000}))
if len(words)!=96000:raise RuntimeError('Incomplete recording; no WAV emitted')
for ch in [0,1]:
 with wave.open(str(folder/('mic-channel-%d.wav'%ch)),'wb') as w:
