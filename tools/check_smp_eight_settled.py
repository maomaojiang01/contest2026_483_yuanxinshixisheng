...
assert not (E/'diagnostic-attempt03.json').exists()
...
with s:
    ...
    (E/'port-settle.bin').write_bytes(settle)
...
(E/'diagnostic-attempt03.bin').write_bytes(buf)
...
(E/'diagnostic-attempt03.json').write_text(json.dumps(result,indent=2)+'\n')
