...
paths=['app/k7radio/skw_netdev.c','app/k7radio/prov_service.inc','tools/sync_sdk.py','board/kickpi_k7/configs/velavision_wifi_ip_local/defconfig','tools/build_wifi_arp.sh','evidence/build/ble-connect-20260909/verification.json']
old=json.loads((R/'private/wifi-arp-original-hashes.json').read_text(encoding='utf-8-sig'))
manifest={p:dict(new=sha(R/p),old=old.get(p)) for p in paths}
(STAGE/'staging.json').write_text(json.dumps(manifest))
remote=r'''
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/home/swl/openvela/work/wifi-arp-stage-20260909')
P=Path('/home/swl/openvela/work/velavision-project')
SDK=P.parents[1];O=SDK/'work/velavision-wifi-arp-20260909';O.mkdir(exist_ok=True)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
...
build=run('build',['bash',str(P/'tools/build_wifi_arp.sh'),str(SDK)])
B=SDK/'cmake_out/velavision_wifi_arp_20260909'
assert 'CONFIG_ARP_SEND_DELAYMSEC=250' in (B/'.config').read_text()
report=dict(revision='wifi-arp-20260909',build_exit_code=0,hardware_tested=False,sync_audit=audit,protocol_tests=test,bridge_tests=bridge,
...
profile='board/kickpi_k7/configs/velavision_wifi_ip_local/defconfig'
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/wifi-arp-20260909/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/wifi-arp-20260909/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/wifi-arp-stage-20260909'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-wifi-arp-20260909/result.tar.gz',str(result)])
