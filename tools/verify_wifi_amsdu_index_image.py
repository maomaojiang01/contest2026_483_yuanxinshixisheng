...
def verify(build):
    build=Path(build).resolve();assert build==ROOT/'artifacts/wifi-amsdu-index-20260909'
    report=json.loads((ROOT/'evidence/build/wifi-amsdu-index-20260909/verification.json').read_text())
    assert report['build_exit_code']==0 and report['handshake_tests']['exit_code']==0
    assert report['wifi_data_tests']['exit_code']==0
    assert report['amsdu_tests']['exit_code']==0
...
if __name__=='__main__':print(verify(ROOT/'artifacts/wifi-amsdu-index-20260909'))
