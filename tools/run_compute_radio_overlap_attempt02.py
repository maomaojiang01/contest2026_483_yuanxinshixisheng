...
async def run(log):
    ...
    try:
        ...
        capture = (E / 'compute-monitor-attempt02.txt').open('xb')
        process = await asyncio.create_subprocess_exec(sys.executable, '-X', 'utf8',
            str(R / 'tools/check_compute_radio_overlap_attempt02.py'), stdout=capture, stderr=asyncio.subprocess.STDOUT)
...
if __name__ == '__main__':
    ...
    with (E / 'ble-overlap-attempt02.jsonl').open('x', encoding='utf-8') as log:
