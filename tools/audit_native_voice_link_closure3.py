...
P=R/'private/native-voice-link-audit3';P.mkdir(exist_ok=True)
O=R/'evidence/native-voice-link-audit3-20260911';O.mkdir(exist_ok=True)
...
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/dev/shm/velavision-link-audit3-20260911')
...
if p.returncode==0:
 ...
 result.update(relocatable_sha256=sha(linked),strong_undefined=len(set(strong)),missing_from_sdk_archives=len(missing),
  ...
  limitation='All common objects with normal archive extraction; conservative symbol inventory; archive membership is not final link proof or runtime safety.')
...
host='swl@192.168.152.131';dest='/dev/shm/velavision-link-audit3-20260911'
