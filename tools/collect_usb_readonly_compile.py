...
R=Path(__file__).resolve().parents[1];E=R/'evidence/build/usb-readonly-20260910'
script=r'''
import json,shlex,subprocess,hashlib
from pathlib import Path
B=Path('/home/swl/openvela/cmake_out/velavision_usb_readonly_20260910')
...
for u in db:
 if not any(u['file'].endswith('/'+n) for n in ['usbhost_storage.c','rk3576_usbhost.c','k7storage_main.c']):continue
...
with (E/'storage-compile.json').open('x',encoding='utf-8') as f:json.dump(rows,f,indent=2)
assert len(rows)==3
for x in rows:
 assert x['preprocess_exit']==x['nm_exit']==0
 for macro in ['__NuttX__ 1','CONFIG_USBHOST_MSC 1','CONFIG_USBHOST_MSC_READONLY 1']:
  assert '#define '+macro in x['macros']
 assert '#define CONFIG_FS_FAT ' not in x['macros']
 assert 'host-mock' not in x['unit']['command']
print('3 actual target storage units verified, MSC read-only on, FAT off')
