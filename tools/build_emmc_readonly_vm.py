...
paths=['tools/sync_sdk.py', 'tools/build_emmc_readonly.sh', 'evidence/build/model-arena-20260910/verification.json', 'board/kickpi_k7/configs/velavision_emmc_readonly_local/defconfig', 'port/new/nuttx/arch/arm64/src/rk3576/rk3576_boot.c', 'app/k7emmc/CMakeLists.txt', 'app/k7emmc/emmc_init.c', 'app/k7emmc/emmc_init.h', 'app/k7emmc/k7emmc_main.c', 'app/k7emmc/Kconfig', 'app/k7emmc/Make.defs', 'app/k7emmc/Makefile', 'app/k7emmc/sdhci_command.c', 'app/k7emmc/sdhci_command.h', 'app/k7emmc/sdhci_control.c', 'app/k7emmc/sdhci_control.h', 'app/k7emmc/sdhci_read.c', 'app/k7emmc/sdhci_read.h', 'app/k7emmc/test_emmc_init.c', 'app/k7emmc/test_sdhci_command.c', 'app/k7emmc/test_sdhci_control.c', 'app/k7emmc/test_sdhci_read.c']
old=json.loads((R/'private/emmc-readonly-original-hashes.json').read_text(encoding='utf-8-sig'))
manifest={p:dict(new=sha(R/p),old=old.get(p)) for p in paths}
(STAGE/'staging.json').write_text(json.dumps(manifest))
remote=r'''
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/home/swl/openvela/work/emmc-readonly-stage-20260910')
P=Path('/home/swl/openvela/work/velavision-project')
SDK=P.parents[1];O=SDK/'work/velavision-emmc-readonly-20260910';O.mkdir(exist_ok=True)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
...
build=run('build',['bash',str(P/'tools/build_emmc_readonly.sh'),str(SDK)])
B=SDK/'cmake_out/velavision_emmc_readonly_20260910'
assert 'CONFIG_EXAMPLES_K7EMMC=y' in (B/'.config').read_text()
report=dict(revision='emmc-readonly-20260910',build_exit_code=0,hardware_tested=False,sync_audit=audit,
...
profile='board/kickpi_k7/configs/velavision_emmc_readonly_local/defconfig'
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/emmc-readonly-20260910/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/emmc-readonly-20260910/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/emmc-readonly-stage-20260910'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-emmc-readonly-20260910/result.tar.gz',str(result)])
