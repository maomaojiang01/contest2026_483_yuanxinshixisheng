...
def main():
    evidence=ROOT/'evidence/build/eapol-rx-20260909'
    result=read(evidence/'verification.json')
    assert result['build_exit_code']==0 and all(t['exit_code']==0 for t in result['tests'])
    for name,item in result['artifacts'].items():
        assert sha(ROOT/'artifacts/eapol-rx-20260909'/name)==item['sha256'],name
    ...
    for item in read(ROOT/'evidence/accepted-source-comparison.json'):
        ...
        if current==item['baseline_sha256']:retained.append(name)
            ...
            assert name in ('app/k7radio/k7radio_main.c','app/k7radio/wifi_assoc_probe.inc'),name
    ...
    report=dict(revision='eapol-rx-20260909',recorded_at=datetime.now(timezone.utc).isoformat(),
        result='PASS',retained_baseline_files=retained,intentional_baseline_changes=changed,
        files=files,build_evidence='evidence/build/eapol-rx-20260909/verification.json',
    ...
    project['latest_development']=dict(revision=report['revision'],
        snapshot='evidence/build/eapol-rx-20260909/development-snapshot.json',
