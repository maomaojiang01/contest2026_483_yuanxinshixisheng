...
FILES=['app/k7host/'+p for p in ['k7_photo.c','k7_photo.h','k7_photo_store.c','k7_photo_store.h']]
FILES+=['app/k7agent/CMakeLists.txt','app/k7agent/cloud/src/board_speech_bridge.c','app/k7agent/cloud/src/board_voice_prompts.inc','app/k7agent/cloud/src/device_photo_prompts.c','app/k7agent/cloud/include/device_photo_prompts.h']
REV='native-photo-prompts-20260915'
...
if args.mode=='audit':
    ...
    (OUT/'sdk.diff').write_text(''.join(diff),encoding='utf8');print(''.join(diff))
...
sdk=pathlib.Path('/home/swl/openvela');root=sdk/'work/velavision-project';ev=root/'evidence/native-photo-prompts-20260915'
...
with (ev/'build.log').open('xb') as log:
 shell='source build/envsetup.sh >/dev/null && prebuilts/tools/cmake/bin/cmake -S nuttx -B cmake_out/velavision_native_photo_prompts_20260915 -G Ninja -DBOARD_CONFIG=kickpi_k7:velavision_cloud_speech_buffered && prebuilts/tools/cmake/bin/cmake --build cmake_out/velavision_native_photo_prompts_20260915 -j4; result=$?; echo $result > work/velavision-project/evidence/native-photo-prompts-20260915/build.exit; exit $result'
...
sdk=pathlib.Path('/home/swl/openvela');ev=sdk/'work/velavision-project/evidence/native-photo-prompts-20260915'
assert (ev/'build.exit').read_text().strip()=='0'
raw=(sdk/'cmake_out/velavision_native_photo_prompts_20260915/nuttx.bin').read_bytes()
