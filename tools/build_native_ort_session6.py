...
R=Path(__file__).resolve().parents[1];P=R/'private/native-ort-session-build6';P.mkdir(exist_ok=True)
O=R/'evidence/native-ort-session-build61-20260911';O.mkdir(exist_ok=True)
remote=r'''
import hashlib,json,shlex,subprocess,tarfile,time
from pathlib import Path
S=Path('/dev/shm/velavision-ort-session-20260911');O=S/'compile-attempt6';O.mkdir()
...
call(['scp.exe',*options,host+':'+dest+'/compile-attempt6/results.tar.gz',str(P/'results.tar.gz')])
