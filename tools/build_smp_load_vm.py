...
import hashlib,json,subprocess,tarfile
from pathlib import Path
S=Path('/home/swl/openvela/work/smp-load-stage-20260910')
P=Path('/home/swl/openvela/work/velavision-project')
SDK=P.parents[1];O=SDK/'work/velavision-smp-load-20260910';O.mkdir(exist_ok=True)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
...
build=run('build',['bash',str(P/'tools/build_smp_load.sh'),str(SDK)])
B=SDK/'cmake_out/velavision_smp_load_20260910'
assert all(x in (B/'.config').read_text().splitlines() for x in ['CONFIG_EXAMPLES_K7LOAD=y','CONFIG_SMP=y','CONFIG_SMP_NCPUS=8','CONFIG_NCPUS=8','CONFIG_RK3576_SMP_DIAG=y','CONFIG_EXAMPLES_K7SMP=y','CONFIG_BOARDCTL_RESET=y','CONFIG_SMP_DEFAULT_CPUSET=0x1','CONFIG_EXAMPLES_K7RADIO_IP=y','CONFIG_BCH_DEVICE_READONLY=y'])
report=dict(revision='smp-load-20260910',build_exit_code=0,hardware_tested=False,sync_audit=audit,
...
profile='board/kickpi_k7/configs/velavision_smp_load_local/defconfig'
...
with tarfile.open(O/'result.tar.gz','w:gz') as t:
 for n in ['nuttx.bin','nuttx','.config']:t.add(B/n,arcname='artifacts/smp-load-20260910/'+n)
 for p in O.iterdir():
  if p.suffix in {'.log','.json'}:t.add(p,arcname='evidence/build/smp-load-20260910/'+p.name)
...
host='swl@192.168.152.131';dest='/home/swl/openvela/work/smp-load-stage-20260910'
...
call(['scp.exe',*options,host+':/home/swl/openvela/work/velavision-smp-load-20260910/result.tar.gz',str(result)])
