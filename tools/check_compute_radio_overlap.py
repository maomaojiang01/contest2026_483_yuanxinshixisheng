...
ble = [json.loads(x) for x in (E / 'ble-overlap.jsonl').read_text().splitlines()]
...
try:
    ...
    (E / 'compute-radio.json').write_text(json.dumps(dict(passed=passed, prompt_returned=prompt,
