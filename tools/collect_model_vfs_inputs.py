...
R=Path(__file__).resolve().parents[1];E=R/'evidence/model-file-vfs-inputs-20260910';E.mkdir(exist_ok=True)
script=r'''
import base64,hashlib,json
from pathlib import Path
N=Path('/home/swl/openvela/nuttx')
paths=[N/'include/sys/stat.h',N/'include/sys/types.h',N/'include/unistd.h']+list((N/'fs/vfs').glob('*stat*.c'))+list((N/'fs/inode').glob('*find*.c'))
out=[]
for p in paths:
