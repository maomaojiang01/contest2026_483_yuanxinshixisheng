...
R=Path(__file__).resolve().parents[1];P=R/'private/native-ort-session-build5';P.mkdir(exist_ok=True)
O=R/'evidence/native-ort-session-build51-20260911';O.mkdir(exist_ok=True)
remote=r'''
import hashlib,json,shlex,subprocess,tarfile,time
from pathlib import Path
S=Path('/dev/shm/velavision-ort-session-20260911');O=S/'compile-attempt5';O.mkdir()
...
try:
 with (O/'build.log').open('wb') as stream:
  p=subprocess.run(['bash','-c','cd /home/swl/openvela && source build/envsetup.sh >/dev/null && '+shlex.join(cmd)],stdout=stream,stderr=subprocess.STDOUT,timeout=1200)
  code=p.returncode
...
call(['scp.exe',*options,host+':'+dest+'/compile-attempt5/results.tar.gz',str(P/'results.tar.gz')])
