...
P = R / 'private/native-asr-link3'
O = R / 'evidence/native-asr-link3-20260911'
...
O=Path('/home/swl/openvela/work/native-sherpa-config2-20260911/link3')
...
result=dict(exit_code=p.returncode,command=cmd,inputs=inputs,firmware_linked=False,model_run=False,asr_ready=False,ort_kernel_set='28-entry ASR config; inference untested')
if p.returncode==0:
...
subprocess.run(['scp.exe',*options,str(P/'audit.py'),host+':'+dest+'/audit-real-asr.py'],check=True)
subprocess.run(['ssh.exe',*options,host,'python3 '+dest+'/audit-real-asr.py'],check=True)
subprocess.run(['scp.exe',*options,host+':/home/swl/openvela/work/native-sherpa-config2-20260911/link3/results.tar.gz',str(P/'results.tar.gz')],check=True)
