...
from verify_smp_load_image import verify
...
previous_build = build.parent / 'smp-load-20260910'
import json
previous_report = json.loads((Path(__file__).resolve().parents[1] / 'evidence/build/smp-load-20260910/verification.json').read_text())
...
assert build.name == 'smp-load-20260910'
...
with s, a.log.open('ab') as log:
    ...
    s.write(b'\x03\r'); s.flush(); time.sleep(.2); s.read(8192)
    ...
    # Preserve original snapshot: destination currently contains a partial candidate.
