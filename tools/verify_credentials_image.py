"""Verify the exact credentials-test build and ELF load bounds before RAM boot."""
import hashlib,json,struct
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
def verify(build):
    build=Path(build).resolve();assert build==ROOT/'artifacts/credentials-20260909'
    report=json.loads((ROOT/'evidence/build/credentials-20260909/verification.json').read_text())
    assert report['build_exit_code']==0 and report['credentials_test']['exit_code']==0
...
if __name__=='__main__':print(verify(ROOT/'artifacts/credentials-20260909'))
