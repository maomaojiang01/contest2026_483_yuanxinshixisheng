...
E=R/'evidence/smp-cross-20260910'
...
passed=returned and any(x=='SMP result=PASS shared_counter=50000 expected=50000' for x in lines)
for cpu in range(5):
...
result=dict(passed=passed,prompt_returned=returned,lines=lines,storage_writes=False,
            scope='four A53 plus one A72, MPIDR/GIC target mapping pinned workers, shared atomic counter and timed sleeps; not eight-core or peripheral SMP acceptance')
