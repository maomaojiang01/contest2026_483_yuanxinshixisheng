...
{
  struct sim *s = ctx;
  if (s->read_error && s->group >= 12 && offset == 0x34) return -1;
...
static int sink_fail;
static int sink(void *arg,uint32_t left,uint32_t right)
{
 (void)arg; assert(left==UINT32_C(0x10000000)+2*sink_count);
 assert(right==left+1); sink_count++; return sink_fail && sink_count==5 ? -125 : 0;
}
int main(void)
{
  struct sim s;
  ...
  initialize(&s); sink_count=0; sink_fail=0;
  assert(pio_run_grouped_sink(&port,1,4096000,capture,256,128,&result,sink,NULL)==0);
  assert(sink_count==128 && !result.held);
  initialize(&s); sink_count=0; sink_fail=1;
  assert(pio_run_grouped_sink(&port,1,4096000,capture,256,128,&result,sink,NULL)==-125);
  assert(sink_count==5 && !result.held && result.stop_result==0);
  initialize(&s); s.mismatch=1; sink_count=0;
  assert(pio_run_grouped_sink(&port,1,4096000,capture,256,128,&result,sink,NULL)==-5);
  assert(sink_count==0 && !result.held);
  initialize(&s); s.read_error=1; sink_count=0; sink_fail=0;
  assert(pio_run_grouped_sink(&port,1,4096000,capture,256,128,&result,sink,NULL)==-5);
  assert(sink_count>0 && sink_count<128 && !result.held);
