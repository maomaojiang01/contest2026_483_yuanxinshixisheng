        ...
        try:
            ...
            while time.monotonic()-begin<a.duration:
                ...
                if remaining!=previous:
                    phase=min(4,int(age/max(1,a.duration/5)))
                    cue=('LEFT / RIGHT','UP / DOWN','REVERSE DIRECTION',
                         'HIDE 2s / RETURN','BOTH AXES / HOLD')[phase]
                    status('active','FOLLOW {}s: {}'.format(remaining,cue))
                    previous=remaining
                response=read()
